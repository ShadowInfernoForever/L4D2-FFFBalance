...
public void OnMapStart() {
	if(gc_bHowl.IntValue == 1) {
		PrecacheSoundAny("halloween/howl.mp3", true);
...
public Action HowlTimer(Handle timer) {
	for(int i = 1; i <= MaxClients; i++) EmitSoundToAllAny("halloween/howl.mp3");
...
public void ExecFog() {
	if(g_iFog != -1) {
		...
		DispatchKeyValueFloat(g_iFog, "fogstart", 550.0);
		DispatchKeyValueFloat(g_iFog, "fogend", 850.0);
