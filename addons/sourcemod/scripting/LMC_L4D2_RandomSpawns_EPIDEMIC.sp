...
#define PLUGIN_NAME "LMC_L4D2_RandomSpawns_EPIDEMIC"
...
void ePlayerSpawn(Handle hEvent, const char[] sEventName, bool bDontBroadcast)
{
    ...
    // Check armor value para filtrar infectados supervivientes
    int armorVal = GetEntProp(iClient, Prop_Send, "m_ArmorValue");
    if (armorVal != 1245) {
        return; // No es el infectado superviviente que buscas
    }
...
{
    int choice = GetRandomInt(LMCModelSectionType_UnCommon, LMCModelSectionType_Common);
    ...
    switch(choice)
    {
        ...
        default:
        {
            return false; // Por si acaso, aunque no debería pasar
        }
    }
