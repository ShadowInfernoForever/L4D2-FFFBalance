...
public void OnPluginStart() {
	...
	Enable.AddChangeHook(Event_ConVarChanged);
	Adrenaline_duration_base.AddChangeHook(Event_ConVarChanged);
	Adrenaline_headshot_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_distance_max.AddChangeHook(Event_ConVarChanged);
	Adrenaline_distance_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_piped_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_melee_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_grenade_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_tank_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_witch_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_boomer_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_smoker_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_hunter_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_spitter_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_jockey_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_charger_ratio.AddChangeHook(Event_ConVarChanged);
	Adrenaline_allow_cooling.AddChangeHook(Event_ConVarChanged);
	Adrenaline_allow_bot.AddChangeHook(Event_ConVarChanged);
	Adrenaline_pills_gain.AddChangeHook(Event_ConVarChanged);
	Adrenaline_duration_full.AddChangeHook(Event_ConVarChanged);
	Adrenaline_combo_time.AddChangeHook(Event_ConVarChanged);
	Adrenaline_combo_ratio.AddChangeHook(Event_ConVarChanged);
...
float GetEventWorth(Event event, float multiplier = 1.0) {
	...
	bool isPiped = StrEqual(weapon, "pipe_bomb");
	bool isMelee = StrEqual(weapon, "melee");
	bool isGrenade = StrContains(weapon, "projectile") != -1;

	float worth = adrenaline_duration_base;
	...
	if (adrenaline_combo_time > 0) {
		...
		worth *= Pow(adrenaline_combo_ratio, combo[attacker] - 1.0);
	}

	if (isInfected(victim))
		switch ( GetEntProp(victim, Prop_Send, "m_zombieClass") ) {
			case SMOKER:	worth *= adrenaline_smoker_ratio;
			case BOOMER:	worth *= adrenaline_boomer_ratio;
			case HUNTER:	worth *= adrenaline_hunter_ratio;
			case SPITTER:	worth *= adrenaline_spitter_ratio;
			case JOCKEY:	worth *= adrenaline_jockey_ratio;
			case CHARGER:	worth *= adrenaline_charger_ratio;
		}

	if (isPiped)
		worth *= adrenaline_piped_ratio;

	if (isHeadshot)
		worth *= adrenaline_headshot_ratio;

	if (isMelee)
		worth *= adrenaline_melee_ratio;

	if (isGrenade)
		worth *= adrenaline_grenade_ratio;

	if (isClient(attacker)) {
		float attacker_pos[3];
		GetClientAbsOrigin(attacker, attacker_pos);
		worth *= 1.0 + (adrenaline_distance_ratio - 1) * ((GetVectorDistance(attacker_pos, victim_pos, false) / adrenaline_distance_max));
	}
...
float AddAdrenaline(int client, float duration, bool force = false) {
	if (isSurvivorAlive(client)) {
		...
		if ( (adrenaline_allow_bot || !IsFakeClient(client)) && (adrenaline_allow_cooling || remaining || force) ) {
			...
			// PrintToChat(client, "remain: %.1f, increase: %.1f", remaining, duration);
...
public void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast) {
	...
	if (isSurvivorAlive(attacker))
		AddAdrenaline(attacker, GetEventWorth(event));
}
...
public void Event_TankDeath(Event event, const char[] name, bool dontBroadcast) {
	...
	if (isSurvivorAlive(attacker))
		AddAdrenaline(attacker, GetEventWorth(event, adrenaline_tank_ratio));
}
...
public void Event_WitchDeath(Event event, const char[] name, bool dontBroadcast) {
	...
	if (isSurvivorAlive(attacker))
		AddAdrenaline(attacker, GetEventWorth(event, adrenaline_witch_ratio));
}
