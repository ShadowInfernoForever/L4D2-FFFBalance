...
{
	g_hTimout = CreateConVar( "l4d2_pause_request_timout", "10.0", "How long the pause request should be visable", 0, true, 5.0, true, 30.0 );
	g_hForcePauseOnly = CreateConVar( "l4d2_pause_force_only", "0", "Only allow force pauses", 0 );
	g_hPauseAllTalk = CreateConVar( "l4d2_pause_alltalk", "1", "Turns Alltalk on when paused", 0 );
...
stock ChatFloodOn()
{
  if (g_hFloodTime != INVALID_HANDLE)
  {
