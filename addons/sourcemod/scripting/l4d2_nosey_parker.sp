...
void PrintInflictedDamage(int iSurvivor, int iInfected)
{
	...
	CPrintToChat(iSurvivor, "{default}[{olive}Daño{default}] le hiciste {olive}%d{default} de daño a {olive}%N{default} ({lightgreen}%s{default})", g_iDamage[iSurvivor][iInfected], iInfected, L4D2_InfectedNames[iZClass]);
