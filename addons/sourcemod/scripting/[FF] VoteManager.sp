...
public Action:Callvote_Handler(client, args)
{
	...
	if (voteInProgress)
	{
		PrintToChat(client, "\x04[VOTACIÓN] \x01 Debes esperar a que el voto términe.");
	...
	if (postVoteDelay)
	{
		PrintToChat(client, "\x04[VOTACIÓN] \x01Must wait %f seconds between votes.", VOTE_DELAY);
	...
	{
	       	PrintToChat(client,"\x04[VOTACIÓN] \x01Invalid vote type: %s",voteName);
	...
	if (isInVoteTimeout(client)){
		...
		PrintToChat(client, "\x04[VOTACIÓN] \x01You must wait %.1f seconds between votes.",GetConVarFloat(voteTimeout));
		return Plugin_Handled;		
	}
	...
	if (hasVoteAccess(client, voteName))
	{
		...
		//PrintToChatAll("\x04[VOTACIÓN] \x01%s initiated a %s vote.", initiatorName, voteName);
		Notify(client, "\x04[VOTACIÓN] \x01%s inició un voto %s.", initiatorName, voteName);
	...
	else
	{
		...
		//PrintToChatAll("\x04[VOTACIÓN] \x01%s tried to start a %s vote but does not have access.", initiatorName, voteName);
		Notify(client, "\x04[VOTACIÓN] \x01%s intentó hacer un voto de %s pero no tiene acceso.", initiatorName, voteName);
		return Plugin_Handled;
	}
...
public Action:Kick_Vote_Logic(client, args)
{
	...
	if ((target<=0) || (!IsClientInGame(target)))
	{
		...
		Notify(client, "\x04[VOTACIÓN] \x01%s intentó hacer un votekickeo a %s pero no es una persona valida.", initiatorName,arg2);
		PrintToChat(client, "\x04[VOTACIÓN] \x01 El Formato Manual es: 'callvote kick <user id>'");
		return Plugin_Handled;
	}
	...
	if (GetConVarBool(tankKickImmunity) && (GetClientTeam(target) == TEAM_INFECTED) && IsPlayerAlive(target))
	{
		...
		if (StrContains(model, "hulk", false) > 0)
		{
			...
			//PrintToChatAll("\x04[VOTACIÓN] \x01%s tried to start a Kick Vote against %s but tanks cannot be kicked.",initiatorName,targetName);
			Notify(client,"\x04[VOTACIÓN] \x01%s Intentó Kickear a %s pero los tanks no pueden ser kickeados.",initiatorName,targetName);
			return Plugin_Handled;
		}
	}
	...
	if (GetClientTeam(client) == TEAM_SPECTATOR)
	{
		...
		//PrintToChatAll("\x04[VOTACIÓN] \x01%s tried to start a Kick Vote against %s but spectators are not allowed to kick.",initiatorName,targetName);
		Notify(client, "\x04[VOTACIÓN] \x01%s Intentó kickear a %s pero los espectadores no tienen permitido.",initiatorName,targetName);
		return Plugin_Handled;
	}
	...
	if (GetConVarBool(kickImmunity))
	{
		...
		if (isAdmin(targetAdminId)){
			...
			if (!CanAdminTarget(clientAdminId, targetAdminId))
			{
				...
				//PrintToChatAll("\x04[VOTACIÓN] \x01%s tried to start a Kick Vote against %s but he has immunity.",initiatorName,targetName);
				Notify(client, "\x04[VOTACIÓN] \x01%s Intentó kickear a %s pero tiene inmunidad.",initiatorName,targetName);
				return Plugin_Handled;
			}
		}
	}
	...
	//PrintToChatAll("\x04[VOTACIÓN] \x01%s is starting a Kick Vote against %s.",initiatorName,targetName);
	Notify(client, "\x04[VOTACIÓN] \x01%s Empezó una votación de kickeo hacia %s.",initiatorName,targetName);
	return Plugin_Continue;
}
...
public Action:Veto_Handler(client, args)
{
	...
	if (!voteInProgress || postVoteDelay) 
	{
		...
		if (client!=0)
		{
			PrintToChat(client, "\x04[VOTACIÓN] \x01No current vote to veto."); 
		}
	...
	if (client==0){
		...
		//PrintToChatAll("\x04[VOTACIÓN] \x01The Server has vetoed this vote.");
		Notify(0, "\x04[VOTACIÓN] \x01The Server has vetoed this vote.");
		return Plugin_Continue;
	...
	{	
		...
		LogVote(client,"has vetoed a vote.");
		//PrintToChatAll("\x04[VOTACIÓN] \x01%s has vetoed this vote.",vetoerName);
		Notify(client, "\x04[VOTACIÓN] \x01%s has vetoed this vote.",vetoerName);
		return Plugin_Continue;
		
	}
	LogVote(client,"failed to veto vote. Reason: Access");
	//PrintToChatAll("\x04[VOTACIÓN] \x01%s tried to veto a vote but does not have access.",vetoerName);
	Notify(client, "\x04[VOTACIÓN] \x01%s tried to veto a vote but does not have access.",vetoerName);
	return Plugin_Handled;
}
...
public Action:PassVote_Handler(client, args)
{
	...
	if (!voteInProgress || postVoteDelay) 
	{
		...
		if (client!=0)
		{
			PrintToChat(client, "\x04[VOTACIÓN] \x01No current vote to pass."); 
		}
		return Plugin_Handled;
	}
	...
	if (client==0){
		...
		//PrintToChatAll("\x04[VOTACIÓN] \x01The Server has passed this vote.");
		Notify(0, "\x04[VOTACIÓN] \x01The Server has passed this vote.");
		return Plugin_Continue;
	...
	{	
		...
		LogVote(client,"has passed a vote.");
		//PrintToChatAll("\x04[VOTACIÓN] \x01%s has passed this vote.",passerName);
		Notify(client, "\x04[VOTACIÓN] \x01%s has passed this vote.",passerName);
		return Plugin_Continue;
		
	}
	LogVote(client,"failed to veto vote. Reason: Access");
	//PrintToChatAll("\x04[VOTACIÓN] \x01%s tried to pass a vote but does not have access.",passerName);
	Notify(client, "\x04[VOTACIÓN] \x01%s tried to pass a vote but does not have access.",passerName);
	return Plugin_Handled;
}
...
public Action:CustomVote_Handler(client, args)
{
	...
	else
	{
		...
		//PrintToChatAll("\x04[VOTACIÓN] \x01%s tried starting a Custom vote but one is already in progress.",initiatorName);
		Notify(client, "\x04[VOTACIÓN] \x01%s Intentó hacer una votación custom pero ya hay una en curso.",initiatorName);
	}
...
public Action:Custom_Vote_Logic(client, args)
{
	...
	if (!customVoteInProgress)
	{
		...
		//PrintToChatAll("\x04[VOTACIÓN] \x01%s is starting a Custom vote.",initiatorName);
		Notify(client, "\x04[VOTACIÓN] \x01%s Empezó una votación customizada.",initiatorName);
...
public Action:Vote_Handler(client, args)
{
	...
	//PrintToChatAll("\x04[VOTACIÓN] \x01%s voted %s.",voterName,vote);	
