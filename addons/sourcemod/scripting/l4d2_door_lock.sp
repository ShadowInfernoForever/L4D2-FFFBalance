...
ConVar Cvar_DoorLock_LoaderMsg;
ConVar Cvar_DoorLock_ExtraBots;
...
{
	if (message[0] == '!' || message[0] == '/') g_bPanelIsOpened[client] = false;
...
{
	if(!g_bGameLeft4Dead || !Cvar_DoorLock_AllowLock.BoolValue || !IsAllowedGameMode())
	{
		g_bIsGameAllowed = false;
		return false;
	}
	
	g_bIsGameAllowed = true;
	return true;
}
...
{
	if(!g_bIsGameAllowed) return;
	
	SafeDeleteTimer(g_hTimer_IgnoreLoaders);
	SafeDeleteTimer(g_hTimer_PendingLoader);
	SafeDeleteTimer(g_hTimer_WarmingUpTime);
	SafeDeleteTimer(g_hTimer_UnreadyGiveUp);
	SafeDeleteTimer(g_hTimer_CountdownTime);
	SafeDeleteTimer(g_hTimer_ReadyUpChecks);
	SafeDeleteTimer(g_hTimer_ForceVersusST);
}
...
public Action OnPlayerRunCmd (int client, int &buttons)
{
	...
	{
		if(IsClientInGame(client) && GetClientTeam(client) == 2 && g_bIsGameAllowed)
...
Action Timer_DisregardLoaders(Handle timer)
{
	...
	if(iHuman < 1) return Plugin_Continue;
...
{
	if(!g_bIsGameAllowed) return Plugin_Handled;
	else if(client == 0) PrintToServer("This command can not be executed by server");
...
{
	if(!g_bIsGameAllowed || !Cvar_DoorLock_EnableRdy.BoolValue) return Plugin_Handled;
	else if(client == 0) PrintToServer("This command can not be executed by server");
...
Action Timer_ActivatePanel(Handle timer, int userid)
{
	...
	if(!IsValidClient(client) || IsFakeClient(client) || GetClientTeam(client) == 1 || !g_bPanelIsOpened[client]) return Plugin_Handled;
...
{
	if(!g_bIsGameAllowed || !g_bLockSafeAreas || g_bLeftSafeAreas) return;
...
{
	if(!g_bIsGameAllowed || !IsValidClient(client)) return;
...
{
	if(!g_bLockSafeAreas || g_bLeftSafeAreas || g_hTimer_WarmingUpTime != null) return Plugin_Handled;
...
{
	if(g_bIsGameAllowed && g_bLockSafeAreas && !g_bLeftSafeAreas && g_iGiveUpsTime > 0)
	{
		Activate_SurvivorTeleport(client, false);
		return Plugin_Handled;
	}
...
{
	if(!g_bIsGameAllowed || !g_bLockSafeAreas || !Cvar_DoorLock_ModesType.BoolValue || !g_bFirstScenario) return;
	
	int client = GetClientOfUserId(event.GetInt("userid"));
	if(!IsValidClient(client) || GetClientTeam(client) != 2) return;
	
	Activate_SurvivorTeleport(client, true);
}
...
void Activate_SurvivorTeleport(int client, bool heal)
{
	if (!IsValidClient(client) || !BlockClientRepetition(client) || GetClientTeam(client) != 2 || IsFakeClient(client)) return;
	...
	if(heal) CreateTimer(0.1, Timer_HealSurvivor, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
}
...
{
	int client = GetClientOfUserId(userid);
	if(IsValidClient(client)) HealSurvivor(client);
	return Plugin_Handled;
}

void HealSurvivor(int client)
{
	if(!IsValidClient(client) || GetClientTeam(client) != 2 || g_bLeftSafeAreas) return;
	...
	if (IsPlayerIncapacitated(client) || IsHandingFromLedge(client))
		L4D_ReviveSurvivor(client);
	
	int iMaxHealth = GetEntProp(client, Prop_Send, "m_iMaxHealth");
	SetEntProp(client, Prop_Send, "m_iHealth", iMaxHealth, 1);
...
void FreezePlayersInFirstChapters()
{
	...
	for (int i = 1; i <= MaxClients; i++)
		if(IsValidEntity(i) && IsValidClient(i) && GetClientTeam(i) == 2)
...
int GetReadyTeamCount(int team)
{
	int number = 0;
	for (int i = 1; i <= MaxClients; i++)
		if(IsValidClient(i) && !IsFakeClient(i) && GetClientTeam(i) == team && g_bClientIsReady[i]) number++;
	return number;
}
...
{
	if(Cvar_DoorLock_ExtraBots.IntValue > 0)
	{
		if(Cvar_DoorLock_ExtraBots.IntValue == (GetRealTeamCount(3) + GetRealTeamCount(2))) return true;
		return false;
	}
...
int GetAdminsCount()
{
	int number = 0;
	for (int i = 1; i <= MaxClients; i++)
		if(IsClientInGame(i) && IsClientGenericAdmin(i)) number++;
	return number;
}
...
int GetLoadingPlayers()
{
	int number = 0;
	for (int i = 1; i <= MaxClients; i++)
		if(IsClientConnected(i) && !IsClientInGame(i) && !IsFakeClient(i)) number++;
	return number;
}
...
int GetRealPlayers()
{
	int number = 0;
	for (int i = 1; i <= MaxClients; i++)
		if(IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) != 1) number++;
	return number;
}
