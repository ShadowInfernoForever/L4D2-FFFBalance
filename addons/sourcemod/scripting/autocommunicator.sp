...
public Action:Command_AutoCommunicate(client, args)
{
    if (client > 0 && client <= MaxClients && IsClientInGame(client)) {
        ...
        CPrintToChat(client, "{green}[{lightgreen}ACB{green}] {default}You are not a valid player.");
    }
}
...
if (!IsClientInGame(client)) {
	...
	if (GetClientTeam(client) == 2) {  // Team 2 = Survivors
        if (isPulled[client]) {
    ...
    else if (GetClientTeam(client) == 3) {  // Team 3 = Infected
        if (isPulling[client]) {
        ...
        CPrintToChat(client, "{green}[{lightgreen}ACB{green}] You cannot use the auto communicator as a spectator.");
    }
}
