...
void HookEvents()
{
	...
	HookEvent("revive_success",				Event_RestoreFirstPerson);
    HookEvent("player_death",				Event_RestoreFirstPersonDeath);
    HookEvent("player_team",				Event_RestoreFirstPerson);
}
...
void UnhookEvents()
{
	...
	UnhookEvent("revive_success",		    Event_RestoreFirstPerson);
    UnhookEvent("player_death",				Event_RestoreFirstPersonDeath);
    UnhookEvent("player_team",				Event_RestoreFirstPerson);
}
...
void Event_RestoreFirstPerson(Event event, const char[] name, bool dontBroadcast)
{
...
void Event_RestoreFirstPersonDeath(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(event.GetInt("userid"));
	if( client && !IsFakeClient(client) &&GetClientTeam(client) == 2 )
	{
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
    ...
    if (buttons == 0)
    {
        ...
        GetClientEyeAngles(client, vDirection);
        GetAngleVectors(vDirection, vAngle, NULL_VECTOR, NULL_VECTOR);

        vPosition[0] += (vAngle[0] * -45);
        vPosition[1] += (vAngle[1] * -45);
        vPosition[2] += (vAngle[2] * -45);

        vDirection[0] *= 1.0;
        vDirection[1] += 0.0;
        vDirection[2] = 0.0;

        TeleportEntity(entity, vPosition, vDirection, NULL_VECTOR);
    }
    else
    {
    	if (entity == INVALID_ENT_REFERENCE)
