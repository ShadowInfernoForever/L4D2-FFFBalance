...
public void OnPluginStart()
{
	...
	g_hCvarMsgPos			= CreateConVar("l4d_incappedpillspop_msg_pos", 				"1", 	"Position of the selfhelp action message (0 - Don't show, 1 - Chat, 2 - Hint, 4 - Center screen (can be combined)", CVAR_FLAGS);
	g_hCvarMsgAdvertPos		= CreateConVar("l4d_incappedpillspop_msgadvert_pos", 		"1", 	"Position of advertise selfhelp keys message (0 - Don't show, 1 - Chat, 2 - Hint, 4 - Center screen (can be combined)", CVAR_FLAGS);
	...
	if( client && buttons & g_iUseButton && (GetEngineTime() - g_fPressTime[client] > 0.5) && g_bEnabled )
	{
		...
		else
		{
			if( g_iShowMsgAll )
			{
				...
				if( g_iMsgPos & MSG_POS_CHAT )		CPrintToChatAllExclude(		client, "\x04%N\x01 %t", client, sItem);
				if( g_iMsgPos & MSG_POS_HINT )		CPrintHintTextToAllExclude(	client, "%N %t", client, sItem);
				if( g_iMsgPos & MSG_POS_CENTER )	CPrintCenterTextAllExclude(	client, "%N %t", client, sItem);
			}
...
public Action Timer_AdvertisePills(Handle timer, any UserId)
{
	...
	if( client && IsClientInGame(client) )
	{
		...
		if( bAllowStandup )
		{
			...
			if( g_iMsgAdvertPos & MSG_POS_CHAT )	CPrintToChat(		client, "%t %t %t", "Press", sKey, sItem);
			if( g_iMsgAdvertPos & MSG_POS_HINT )	CPrintHintText(		client, "%t %t %t", "Press", sKey, sItem);
			if( g_iMsgAdvertPos & MSG_POS_CENTER )	CPrintCenterText(	client, "%t %t %t", "Press", sKey, sItem);
		}
	}
	return Plugin_Continue;
}
