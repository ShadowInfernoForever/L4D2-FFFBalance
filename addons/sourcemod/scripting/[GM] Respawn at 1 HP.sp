...
public void OnPluginStart()
{
	...
	if ( bL4D2 ) {
		...
		ThrownWeapon 		= CreateConVar("l4d_survivorrespawn_thrownweapon", 		"0", 	"Which is thrown weapon will be given to the Survivor (1 - Moltov, 2 - Pipe Bomb, 3 - Bile Jar, 4 - None)", FCVAR_NOTIFY, true, 1.0, true, 4.0);
...
public Action OnPluginStart_Delayed( Handle hTimer )
{
	if ( hCvar_Enable.BoolValue )
		bRespawn = true;
	
	if ( hCvar_IncludeBots.BoolValue )
		bIncludeBots = true;
	
	if ( hCvar_RespawnHanging.BoolValue )
		bIncludeHanging = true;

	if ( hCvar_RespawnIncapped.BoolValue )
		bRespawnIncapped = true;

	if ( hCvar_RespawnRespect.BoolValue )
		bEnablesRespawnLimit = true;
...
public void Event_PlayerDeath( Event hEvent, const char[] sName, bool bDontBroadcast )
{
	...
	if ( bRespawn && !bEnablesRespawnLimit && IsValidClient( client ) )
	{
...
//	int client = GetClientOfUserId( hEvent.GetInt( "player" ) );
	...
	if ( bRespawn && !bEnablesRespawnLimit && IsValidClient( bot ) )
	{
...
void SetHealth( int client )
{
	...
	SetEntProp(client, Prop_Send, "m_iMaxHealth", 1);
