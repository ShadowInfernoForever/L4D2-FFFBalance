...
new String:OriginalName[10][MAX_NAME_LENGTH];
...
public OnPluginStart()
{
	...
	PlayerJoinMessageLayout = CreateConVar("PACT_Player_Join_Message_Layout", "{NAME} from {CITY} in {LOC} has joined.", "Format of the welcome message, {NAME} = player name, {LOC} = country, {CITY} = city", CVAR_FLAGS);
	...
	PACTLIST_Layout = CreateConVar("PACT_List_Layout", "{NAME} is from {CITY} in {LOC}", "Layout of how !pactlist should be displayed", CVAR_FLAGS);
		...
		if( !IsFakeClient( client ) )
		{
			...
			if( GetConVarBool(PlayerJoinMessage) )
			{
				if( GeoipCountry( IP, Country, 100 ) )
				{
					...
					if ( (StrContains(JoinMessage, "{CITY}", false) != -1) ) 
					{
						if( ( StrEqual( ClientCity[client], "" )) && GetUserAdmin( client ) != INVALID_ADMIN_ID )
						{
							PrintToChat( client, "\x03PACT ERROR!" );
							PrintToChat( client, "\x03You have used {CITY} in your cfg file" );
							PrintToChat( client, "\x03But the Geocity database was not found" );
						}
						ReplaceString(JoinMessage, sizeof(JoinMessage), "{CITY}", ClientCity[client]);
					}
			...
			new String:NameWithTag[MAX_NAME_LENGTH];
		...
		if( !IsFakeClient( client ) )
		{
			for( new i = 1; i<=MaxClients; i++)
			{
				if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i))
				{
					...
					if( StrContains( Layout, "{CITY}", false ) != -1 )
					{
						ReplaceString( Layout, 256, "{CITY}", ClientCity[i], false );
					}
