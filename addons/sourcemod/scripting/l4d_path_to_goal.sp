...
public void OnPluginStart()
{
    ...
    // English Commands
    RegConsoleCmd("sm_path",             CmdRequestGuide, "Show path to the objective.");
    RegConsoleCmd("sm_goal",             CmdRequestGuide, "Show path to the objective.");
    RegConsoleCmd("sm_path_goal",        CmdRequestGuide, "Show path to the objective.");
    RegConsoleCmd("sm_lost",        CmdRequestGuide, "Show path to the objective.");
    RegConsoleCmd("sm_ptg",              CmdRequestGuide, "Show path to the objective.");

    // Spanish Commands
    RegConsoleCmd("sm_camino",           CmdRequestGuide, "Muestra el camino al objetivo.");
    RegConsoleCmd("sm_objetivo",         CmdRequestGuide, "Muestra el camino al objetivo.");
    RegConsoleCmd("sm_guia",             CmdRequestGuide, "Muestra el camino al objetivo.");
    RegConsoleCmd("sm_ruta", CmdRequestGuide, "Muestra el camino al objetivo.");
    
    RegAdminCmd("sm_l4d_path_to_goal_recalculate", CmdRecalculate, ADMFLAG_ROOT,"Recalculate guide points.");
    ...
    g_hCvarSpec = CreateConVar("l4d_path_to_goal_spec", "0.0",
...
Action CmdRequestGuide(int client, int args)
{
    ...
    switch (RequestGuide(client,duration,backward))
    {
        case true: // beams drawn
        {
            ...
            {
                ReplyToCommand(client, "\x04➜\x01 %t%t", "ptg_look", "ptg_down");
            ...
            ReplyToCommand(client, "\x04➜\x01 %t%s %s", "ptg_look", str1, str2);
        }
        default: // beams not drawn
        {
            if (!guide_ready && g_CellRequests[client].duration > 0.0) ReplyToCommand(client, "➜➜➜ %t", "ptg_wait");
        }
    }
    return Plugin_Continue;
}
...
Action CmdRecalculate(int client, int args)
{
    ...
    else ReplyToCommand(client, "\x04➜\x01 %t", "ptg_busy");
    return Plugin_Continue;
}
