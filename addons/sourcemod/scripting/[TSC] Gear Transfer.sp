...
#define PLUGIN_VERSION		"2.33 - modded by little_froy"
...
void Event_PlayerShoved(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( IsFakeClient(client) || SurvivorArena_GetTeam(client) != SurvivorArena_GetTeam(target))
		return;
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if( buttons & IN_RELOAD || (buttons & IN_ATTACK2 && !(buttons & IN_ATTACK)) )
	{
		...
		if( target == -1 || target > MaxClients || SurvivorArena_GetTeam(client) != SurvivorArena_GetTeam(target) || GetClientTeam(target) != 2 || !IsPlayerAlive(target) )
			return Plugin_Continue;
					...
					if( IsValidEntRef(entity) == true && GetEntPropEnt(entity, Prop_Send, "m_hOwnerEntity") == bot )
					{
						...
						for( player = 1; player <= MaxClients; player++ )
						{
							...
							if( targetValid[player] == 0 )
							{
								...
								if(SurvivorArena_GetTeam(bot) == SurvivorArena_GetTeam(player) && IsClientInGame(player) && GetClientTeam(player) == 2 && IsPlayerAlive(player) && IsFakeClient(player) == false && !IsReviving(player) && !IsIncapped(player) )
								{
