...
public OnPluginStart()
{
	...
	h_cvarSpeedBoost = CreateConVar("l4d2_tankfire_boost_amount", "1.20", "Multiplier for tank speed while on fire.", FCVAR_PLUGIN, true, 0.5, true, 2.0);
	h_cvarWarningEnabled = CreateConVar("l4d2_tankfire_warning_enable", "0", "1 prints a warning to the screen when the tank is lit. 0 does not.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	{
		char soundPath[64];
		Format(soundPath, sizeof(soundPath), "player/tank/voice/pain/tank_fire_0%d.wav", i);
		PrecacheSound(soundPath);
	}
	
	if(boost_enabled) ModuleEnabled();
}
...
ModuleDisabled()
{
	...
	UnhookEvent("player_spawn", EventTanks);
	UnhookEvent("player_disconnect", EventTanks);
	UnhookEvent("player_death", EventTanks);
	
	UnhookEvent("player_bot_replace", EventTanks);
	UnhookEvent("bot_player_replace", EventTanks);
	UnhookEvent("player_team", EventTanksDelayed);
...
public OnPluginEnd()
{
	...
	//tanks need to slow down
	if(tankCount != 0)
	{
		//slow any speedy tanks
		new index = 0;
	...
	{	
		...
		if (IsPlayerOnFire(index))
		{
			if (!isFast[index])
			{
				...
				int randomSound = GetRandomInt(1, 8);
				char sound[64];
				Format(sound, sizeof(sound), "player/tank/voice/pain/tank_fire_0%d.wav", randomSound);
				
				// Emit sound from the tank's location
				EmitSoundToAll(sound, index, SNDCHAN_REPLACE, SNDLEVEL_ROCKET, 0, 1.0, 100, 200);
			}
		}
...
public ChangeVars(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	...
	if(boost_enabled == 0 && old_boost == 1)
	{
		new index = 0;
		for(new i = 1; i <= tankCount; i++)
		{	
	...
	else if(boost_enabled == 1 && old_boost == 0)
	{
		ModuleEnabled();
	}
