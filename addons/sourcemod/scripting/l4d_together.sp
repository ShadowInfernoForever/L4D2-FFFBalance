...
public OnPluginStart()
{
	...
	l4d_together_enable= 		CreateConVar("l4d_together_enable", "1", " 0:diable  , 1: enable");
 	l4d_together_loner_range=	CreateConVar("l4d_together_loner_range", "2000.0", "if some one away from his teamate, he is the loner"); 
	...
	l4d_safedoor_require_count= CreateConVar("l4d_together_saferoom_require_count", "100", "min count of suvivors require to operate saferoom , 0:disable, survivors can operate saferoom in any condition");
	l4d_safedoor_require_notank= CreateConVar("l4d_together_safedoor_require_notank", "1", "1:can not opearte safe door with tank alive; 0: ignore tank");
	l4d_safedoor_ignore_bot= CreateConVar("l4d_together_safedoor_ignore_bot", "1", "1:ignore bot ; 0:do not ignore bot");

 	
	AutoExecConfig(true, "l4d_together");	
		...
		{ 
			if(StrEqual(entname, "prop_door_rotating_checkpoint"))
			{
				...
				if((IsLeftStartAreaArea() ) && GetConVarInt(l4d_safedoor_require_notank)==1 && HaveTank())
				{
					...
					PrintToChat(client, "We must kill all tank before operate the door");
				}
				else 
				{
...
public Action:TimerUpdatePlayer(Handle:timer, any:data)
{
	...
	else 
	{
		if(Msg[PlayerIndex])
		{
			PrintHintText(PlayerIndex, "Con Cuidado, Estás muy lejos de tu equipo"); 
		}
