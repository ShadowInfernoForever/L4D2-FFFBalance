...
public void OnPluginStart()
{
	...
	CreateConVar("l4d_vote_restart_round_version", "1.6", _, FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
Action RestartRound(int client, int args)
{
	...
	{
		if(iLastRoundNumber != -1) return Plugin_Handled; // A restart in progress, jump out
		iLastRoundNumber = LoadFromAddress(L4D_GetPointer(POINTER_DIRECTOR)+view_as<Address>(m_iRoundNumber), NumberType_Int8);
		StoreToAddress(L4D_GetPointer(POINTER_DIRECTOR)+view_as<Address>(m_iRoundNumber), 0, NumberType_Int8);
		// if(e == Engine_Left4Dead2) StoreToAddress(L4D_GetPointer(POINTER_DIRECTOR)+view_as<Address>(m_iRoundNumber), e == Engine_Left4Dead2 ? 1 : 0, NumberType_Int8);
		// else StoreToAddress(L4D_GetPointer(POINTER_DIRECTOR)+view_as<Address>(m_iRoundNumber), e == Engine_Left4Dead2 ? 1 : 0, NumberType_Int8);
...
public void round_start_post_nav(Event event, char[]name, bool dontBroadcast)
{
	...
	{
		StoreToAddress(L4D_GetPointer(POINTER_DIRECTOR)+view_as<Address>(m_iRoundNumber), iLastRoundNumber, NumberType_Int8)
		EngineVersion e = GetEngineVersion();
		if(e == Engine_Left4Dead2)
		{
			for(int j=0;j<8;j++)
			{
				GameRules_SetProp("m_iVersusDistancePerSurvivor", 0, _, j)
				GameRules_SetProp("m_iVersusSurvivorDeathDistance", 0, _, j)
				GameRules_SetProp("m_iScavengeTeamScore", 0, _, j)
			}
			GameRules_SetProp("m_iScavengeTeamScore", 0, _, 8)
			GameRules_SetProp("m_iScavengeTeamScore", 0, _, 9)
			GameRules_SetProp("m_iScavengeMatchScore", 0, _, iLastRoundNumber)
			GameRules_SetPropFloat("m_flTeamRoundTime", 0.0, iLastRoundNumber)
			GameRules_SetPropFloat("m_flRoundDuration", 0.0, iLastRoundNumber)
			GameRules_SetProp("m_iSurvivorScore", 0, _, iLastRoundNumber)
			GameRules_SetProp("m_iCampaignScore", 0, _, iLastRoundNumber)
			GameRules_SetProp("m_iChapterScore", 0, _, iLastRoundNumber)
			GameRules_SetProp("m_iChapterDamage", 0, _, iLastRoundNumber)
			GameRules_SetProp("m_iVersusDefibsUsed", 0, _, iLastRoundNumber)
			GameRules_SetProp("m_iVersusSurvivalMultiplier", 0, _, iLastRoundNumber)
		}
		else
		{
			GameRules_SetProp("m_iSurvivorScore", 0, _, iLastRoundNumber)
			GameRules_SetProp("m_iCampaignScore", 0, _, iLastRoundNumber)
			GameRules_SetProp("m_iVersusDistance", 0)
			GameRules_SetProp("m_iVersusCompletion", 0)
			GameRules_SetProp("m_iVersusHealthBonus", 0)
			GameRules_SetProp("m_iVersusSurvivalMultiplier", 0)
		}
		iLastRoundNumber = -1
		hRestartVersus.Disable(); // For safety
	}
}
