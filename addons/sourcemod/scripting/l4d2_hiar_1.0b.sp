...
saystats(attacker)
{
	if (GetConVarInt(l4d2_hiar) == 1) 
	{
		...
		{ 
			PrintToChatAll("\x04[!hiar]\x01 \x05%N\x01, hizo \x04%i\x01 disparos a la cabeza seguidos", attacker, headcountrow[attacker]);
		...
		if (GetConVarInt(l4d2_hiarnfows) == 1)
		{
			...
			PrintToChatAll("\x04[!hiar] \x05%N\x01 en \x04%i\x01 Disparos a la cabeza: \x05%i\x01/%i \x05hits\x01 %d%% \x04| \x03%i\x01/%i \x03bajas\x01 %d%", attacker, headcountrow[attacker], shdShot_check, skill_check, shdShotAcheck, hdShot_check, kill_check, hdShotAcheck);
