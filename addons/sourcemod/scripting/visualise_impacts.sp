...
{
	g_cvDecal = CreateConVar("vi_decal", "materials/decals/shadow/circle01.vtf", "Ruta del decal a usar", FCVAR_NOTIFY);
    HookConVarChange(g_cvDecal, OnDecalChanged);
    char sDecalPath[PLATFORM_MAX_PATH];
	GetConVarString(g_cvDecal, sDecalPath, sizeof(sDecalPath));
	g_iPrecacheDecal = PrecacheDecal(sDecalPath, true);
...
public void OnDecalChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
    g_iPrecacheDecal = PrecacheDecal(newValue, true);
}
