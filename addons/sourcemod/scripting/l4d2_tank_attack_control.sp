...
public void TankSpawn_Event(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		CPrintToChat(tank, "{blue}[{default}Selector de Rocas del TANK{blue}]");
		CPrintToChat(tank, "{olive}Recargar {default}= {blue}2 Manos Superior");
		CPrintToChat(tank, "{olive}Usar/Agarrar {default}= {blue} Por Debajo");
		CPrintToChat(tank, "{olive}M2 {default}= {blue}1 Mano Superior");
	}
}
