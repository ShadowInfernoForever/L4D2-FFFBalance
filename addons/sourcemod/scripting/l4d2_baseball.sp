...
public void OnPluginStart()
{
	...
	sv_melee_radius_projectile = CreateConVar("sv_melee_radius_projectile", "100.0");
	sv_melee_deflect_filter = CreateConVar("sv_melee_deflect_filter", "molotov_projectile, pipe_bomb_projectile, vomitjar_projectile, spitter_projectile" );
	...
	if ( TR_DidHit () )
	{
		...
		if ( entity != 0 )
		{
			...
			if ( StrContains(szName, "_projectile") != -1 && IsDeflectable(szName) )
			{
				...
				PrintToChatAll("\x04%N \x03bateó el proyectil con una velocidad de \x04%.2f \x03!", client, GetVectorLength(vVelocity));
				return Plugin_Stop;
			}
		}
	}
