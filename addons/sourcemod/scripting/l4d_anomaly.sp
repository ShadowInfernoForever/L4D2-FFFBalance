...
void DoDamage(int entity, int client, float vPos[3], int type = 0)
{
	...
	switch( type )
	{
		case 2:		SDKHooks_TakeDamage(client, entity, entity, float(g_iCvarDamageSurv), DMG_DISSOLVE, -1, NULL_VECTOR, vEnd); // Survivor
		case 3:		SDKHooks_TakeDamage(client, entity, entity, float(g_iCvarDamageSpec), DMG_DISSOLVE, -1, NULL_VECTOR, vEnd); // Special Infected
		case 4:		SDKHooks_TakeDamage(client, entity, entity, float(g_iCvarDamageInfe), g_bLeft4Dead2 ? DMG_DISSOLVE : DMG_BLAST, -1, NULL_VECTOR, vEnd);	// Common L4D2 / L4D1
