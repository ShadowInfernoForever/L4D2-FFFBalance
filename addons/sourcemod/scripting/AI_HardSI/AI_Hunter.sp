...
void ability_use_OnPounce(int botHunter) {	
	...
	if( GetVectorDistance(hunterPos, impactPos) < g_iCvarWallDetectionDistance ) {
	// 20% of probability to pounce straight to the target
	if (GetRandomFloat(0.0, 1.0) < 0.2) {
		#if DEBUG_HUNTER_AIM
			PrintToChatAll("Decided not to jump sideways.");
		#endif
		return; // Don't do angle lunge
	}

	// 80% probability to pounce sideways
	if (GetRandomInt(0, 1)) {
	    ...
	    // Si NO lo están mirando, y hay una probabilidad, hace un super salto vertical
	    if (!IsTargetWatchingAttacker(botHunter, g_fCvarAimOffsetSensitivity) && GetRandomFloat(0.0, 1.0) < 0.15) {
	        float verticalLunge[3];
	        verticalLunge[Z] = 800.0; // Salto alto hacia arriba

	        SetEntPropVector(entLunge, Prop_Send, "m_queuedLunge", verticalLunge);

	        #if DEBUG_HUNTER_AIM
	            PrintToChatAll("Hunter hizo un super salto vertical porque nadie lo mira");
	        #endif

	        return;
	    }

	    // Si lo están mirando, hace pounce con ángulo para despistar
	    GetClientAbsOrigin(botHunter, hunterPos);
	    int target = GetClientAimTarget(botHunter);
	    if (IsTargetWatchingAttacker(botHunter, g_fCvarAimOffsetSensitivity) && GetSurvivorProximity(hunterPos) > g_iCvarStraightPounceProximity && (IsSurvivor(target) && !IsIncapacitated(target)) ) {
...
static void LimitLungeVerticality( int lungeEntity ) {
	...
	float vertAngle = GetRandomFloat(5.0, 22.0);
	// Get the original lunge's vector
	float lungeVector[3];
	GetEntPropVector(lungeEntity, Prop_Send, "m_queuedLunge", lungeVector);
	float x = lungeVector[X];
