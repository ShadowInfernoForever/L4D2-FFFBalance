...
static ConVar g_hCvarEnable, g_hChargerBhop,
	...
	g_hChargeMaxSpeed, g_hChargeStartSpeed, g_hCvarPredict;
...
static bool g_bCvarEnable, g_bChargerBhop, g_bPredictCharger;
...
void Charger_OnModuleStart() {
    ...
    g_hCvarPredict = CreateConVar("ai_charger_predict", "1", "Enable charger predictive aiming", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
static void GetCvars()
{
	...
	g_bPredictCharger = g_hCvarPredict.BoolValue;
}
...
stock Action Charger_OnPlayerRunCmd(int charger, int &buttons) {
	...
	if (IsGrounded(charger)) {
		...
		if (CurTargetDistance(charger) > 500.0 && nearestSurDist > -1.0 && nearestSurDist < 1500) {
...
void ability_use_OnCharge(int charger) {
	...
	if( !IsSurvivor(aimTarget) || IsTargetWatchingAttacker(charger, g_fCvarAimOffsetSensitivity) ) {	
		...
		if( newTarget != -1 && distance != -1 && distance <= g_hCvarChargeProximity.IntValue ) {
			...
			if (!g_bPredictCharger) return;
			UpdateChargerLookAtTarget(charger);
		}
	}
}
...
void UpdateChargerLookAtTarget(int charger)
{
	if (!IsBotCharger(charger)) return;

	float chaPos[3], surPos[3], surEye[3], surVel[3];
	int target = GetClosestSur(charger, 1500.0); // find any close survivor

	if (!IsSurvivor(target) || IsIncapacitated(target)) return;

	GetClientEyePosition(target, surEye);
	GetClientAbsOrigin(target, surPos);
	GetClientAbsOrigin(charger, chaPos);
	GetEntPropVector(target, Prop_Data, "m_vecVelocity", surVel);

	// Predict where the survivor will be
	float dx = surPos[0] - chaPos[0];
	float dy = surPos[1] - chaPos[1];
	float dz = surPos[2] - chaPos[2];
	float vx = surVel[0];
	float vy = surVel[1];
	float vz = surVel[2];

	float A = vx*vx + vy*vy + vz*vz - 500.0*500.0;
	...
	if (discriminant >= 0.0 && FloatAbs(A) > 0.001)
	{
		float sqrtDisc = SquareRoot(discriminant);
		float t1 = (-B + sqrtDisc) / (2*A);
		float t2 = (-B - sqrtDisc) / (2*A);
		predictTime = t1 > t2 ? t1 : t2;
	}

	// Si la predicción es razonable, ajustar con la velocidad del survivor
	if (predictTime > 0.225)
	{
		surEye[0] += surVel[0] * (predictTime + 0.3);
		surEye[1] += surVel[1] * (predictTime + 0.3);
		surEye[2] += surVel[2] * (predictTime + 0.3);
	}

	float dir[3];
	MakeVectorFromPoints(chaPos, surEye, dir);
	float angle[3];
	GetVectorAngles(dir, angle);
	TeleportEntity(charger, NULL_VECTOR, angle, NULL_VECTOR);
}
