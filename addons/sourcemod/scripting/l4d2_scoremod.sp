...
void SM_RoundEnd_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!SM_bIsFirstRoundOver) 
	{
		...
		CPrintToChatAll("{blue}[{default}!{blue}] {default}Ronda {blue}1 {default}Bonus: {olive}%d", SM_iFirstScore);
	...
	else if (SM_bIsSecondRoundStarted && !SM_bIsSecondRoundOver)
	{
		...
		CPrintToChatAll("{blue}[{default}!{blue}] {default}Ronda {blue}1 {default}Bonus: {olive}%d", SM_iFirstScore);
		CPrintToChatAll("{blue}[{default}!{blue}] {default}Ronda {blue}2 {default}Bonus: {olive}%d", iScore);
		iDifference = SM_iFirstScore - iScore;
		if (iScore > SM_iFirstScore) iDifference = (~iDifference) + 1;
		CPrintToChatAll("{red}[{default}!{red}] {default}Diferencia: {olive}%d", iDifference);
		if (GetConVarBool(SM_hCustomMaxDistance) && GetCustomMapMaxScore() > -1) CPrintToChatAll("{blue}[{default}!{blue}] {default}Distancia Máx. Personalizada: {olive}%d", GetCustomMapMaxScore());
	}
}
...
Action SM_Cmd_Health(client, args)
{
	...
	if (SM_bIsSecondRoundStarted)
    {
		iDifference = SM_iFirstScore - iScore;
		if (iScore > SM_iFirstScore) iDifference = (~iDifference) + 1;
		CPrintToChat(client, "{blue}[{default}!{blue}] {default}Ronda {blue}1 {default}Bonus: {olive}%d {default}({green}Diferencia: {olive}%d{default})", SM_iFirstScore, iDifference);
	}
	...
	if (client)
	{
		CPrintToChat(client, "{blue}[{default}!{blue}] {default}Bonus de Vida: {olive}%d", iScore );
	}
	else
	{
	...
	if (GetConVarBool(SM_hCustomMaxDistance) && GetCustomMapMaxScore() > -1) {
      if (client) {
        CPrintToChat(client, "{blue}[{default}!{blue}] {default}Distancia Máx. Personalizada: {olive}%d", GetCustomMapMaxScore());
    }
    else {
        PrintToServer("[ScoreMod] Distancia Máx. Personalizada: %d", GetCustomMapMaxScore());
    }
  }
