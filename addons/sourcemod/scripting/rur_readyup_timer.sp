...
public OnFirstClientLoaded() {

	OnReadyUpEndBlock();	// This plugin handles when the ready up period ends, so I don't want the ready up core plugin to fire OnReadyUpEnd() until this plugin tells it to.
}
...
public Action:timer_ReadyUpEnd(Handle:timer) {
	...
	if (bIsReadyUp) {	//	Display a hint message to players about ready up time stuffs.
		...
		if (i_ReadyUpTime < 1 || readyPlayers == totalPlayers) {	// Prevent players from changing their ready status and force the core plugin to fire OnReadyUpEnd.
