...
#define CONFIG_DATA		"data/scavengebots.cfg"
...
{
	name = "[L4D2] ScavengeBots",
	author = "Machine",
	description = "Survivor Bots Scavenging.",
	version = PLUGIN_VERSION,
	url = "https://forums.alliedmods.net/showthread.php?p=2204131#post2204131"
}
...
stock BotAI(client)
{
	if (IsBot(client) && bScavengeInProgress)
	{
			...
			if (!IsPlayerHeld(client) && !IsPlayerIncap(client))
			{
				if (BotTarget[client] > 0)
				{
					...
					if (IsGasCan(entity))
					{
						...
						ScriptCommand(client, "script", "CommandABot({cmd=1,pos=Vector(%f,%f,%f),bot=GetPlayerFromUserID(%i)})", TOrigin[0], TOrigin[1], TOrigin[2], GetClientUserId(client));
						BotAction[client] = 1;
				...
				else
				{
					...
					if (storedent > 0 && IsValidGasCan(storedent) && !IsGasCanOwned(storedent))
					{
						...
						ScriptCommand(client, "script", "CommandABot({cmd=1,pos=Vector(%f,%f,%f),bot=GetPlayerFromUserID(%i)})", SOrigin[0], SOrigin[1], SOrigin[2], GetClientUserId(client));
						BotAction[client] = 1;
						BotAIUpdate[client] = 10;
						BotUseGasCan[client] = -1;
						BotAbortTick[client] = 50;
		...
		else if (BotAction[client] == 2)
		{
			...
			{
				ScriptCommand(client, "script", "CommandABot({cmd=1,pos=Vector(%f,%f,%f),bot=GetPlayerFromUserID(%i)})", NozzleOrigin[0], NozzleOrigin[1], NozzleOrigin[2], GetClientUserId(client));
				BotAction[client] = 3;
				BotAIUpdate[client] = 10;
				BotAbortTick[client] = 50;
				GetClientAbsOrigin(client, BotCheckPos[client]);
