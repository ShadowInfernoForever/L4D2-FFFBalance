...
public OnPluginStart()
{
	...
	g_cvarEnable = 				CreateConVar("l4d2_undoff_enable", 		"7", 	"Bit flag: Enables plugin features (add together): 1=too close, 2=Charger carry, 4=guilty bots, 7=all, 0=off", CVAR_FLAGS);
	g_cvarAnnounce = 			CreateConVar("l4d2_undoff_announce", 	"2", 	"Bit flag: Enable damage announcements: 2=chat, 1=console, 0=off", CVAR_FLAGS);
	g_cvarBlockZeroDmg =		CreateConVar("l4d2_undoff_blockzerodmg","5", 	"Bit flag: Block 0 damage friendly fire effects like recoil and vocalizations/stats (add together): 4=bot hits human block recoil, 2=block vocals/stats on ALL difficulties, 1=block vocals/stats on everything EXCEPT Easy (flag 2 has precedence), 0=off", CVAR_FLAGS);
...
public Action:OnTakeDamageUndoFF(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
	...
	// Only interested in Survivor victims
	if (!IS_VALID_SURVIVOR(victim) && !L4D_IsPlayerIncapacitated(victim)) return Plugin_Continue;
