...
public SMCResult:Config_KeyValue(Handle:parser, const String:key[], const String:value[], bool:ley_quotes, bool:value_quotes)
{
	...
	else if (StrEqual(key, "all other rounds")) i_OtherMapRounds = StringToInt(value);
	else if (StrEqual(key, "ready up time?")) i_ReadyUpTime = StringToInt(value);
	else if (StrEqual(key, "survival gametype?")) i_IsSurvivalGame = StringToInt(value);
	else if (StrEqual(key, "coop gametype?")) i_IsCoopGame = StringToInt(value);
	else if (StrEqual(key, "versus gametype?")) i_IsVersusGame = StringToInt(value);
	...
	else if (StrEqual(key, "delete saferoom door on round start?")) i_IsDeleteSaferoomDoor = StringToInt(value);
	else if (StrEqual(key, "majority ready counter?")) i_IsMajorityCounter = StringToInt(value);
