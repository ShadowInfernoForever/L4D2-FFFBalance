...
#define TANK_ROCK_CLASSNAME "tank_rock"
#define TANK_ROCK_HIT_SOUND "weapons/tank_rock_impact.wav"
...
{
	PrecacheSound(TANK_ROCK_HIT_SOUND, true);
...
public void entity_shoved(Event event, const char[] name, bool dontbroadcast)
{
    ...
    char szName[MAX_PROJECTILE_NAME];
    GetEntityClassname(entity, szName, sizeof szName);

    if (strcmp(szName, TANK_ROCK_CLASSNAME) == 0)
    {
        float vVelocity[3];
        int attacker = event.GetInt("userid");

        // Get attacker's position and calculate force
        if (attacker > 0)
        {
            attacker = GetClientOfUserId(attacker);
            if (IsClientInGame(attacker))
            {
                vVelocity = CalculateBaseForce(entity, attacker);
                ...
                EmitSoundToAll(TANK_ROCK_HIT_SOUND, SOUND_FROM_WORLD, .origin = NULL_VECTOR);
                PrintToChatAll("\x04%N \x03punched the tank's rock!", attacker);
                return;
            }
        }
    }
    else if (StrContains(szName, "_projectile") != -1)
    {
        float vVelocity[3];
...
// Modify IsDeflectable to include tank rock
bool IsDeflectable(const char[] name)
{
    ...
    return (StrContains(filter, name, false) != -1 || strcmp(name, TANK_ROCK_CLASSNAME) == 0);
}
