    ...
    public Plugin myinfo = 
    {
        ...
        author      = "Jahze (Updated by Shadow)",
        description = "Allows tweaking of the attributes of all weapons (Left4DHooks compatible)",
        version     = "1.3"
    };
    ...
    #define PLUGIN_WEAPON_MAX_ATTRS     (GAME_WEAPON_MAX_ATTRS + 3) // Including: tankdamagemult(Tank damage multiplier), reloaddurationmult(Reload duration multiplier), headshotmultiplier the plugin is responsible for these attributes
    ...
    static const char sWeaponAttrNames[PLUGIN_WEAPON_MAX_ATTRS][MAX_ATTRS_NAME_LENGTH] = 
    {
        ...
        "Headshot damage multiplier", // the plugin is responsible for this attribute
    };
    ...
    static const char sWeaponAttrShortName[PLUGIN_WEAPON_MAX_ATTRS][MAX_ATTRS_NAME_LENGTH] =
    {
        ...
        "headshotmult", // the plugin is responsible for this attribute
    };
    ...
    Handle g_hTankDamageKVs = INVALID_HANDLE;
    Handle g_hHeadshotKVs = INVALID_HANDLE;
    ...
    public void OnPluginStart()
    {
        ...
        g_hTankDamageKVs = CreateKeyValues("DamageVsTank");
        g_hHeadshotKVs = CreateKeyValues("HeadshotMultiplier");
        ...
        for (int i = 1; i <= MaxClients; i++)
        {
            if (IsClientInGame(i))
            {
                ...
                SDKHook(i, SDKHook_TraceAttack, OnTraceAttackHeadshot);
            }
        }
    }
    ...
    public void OnClientPutInServer(int client)
    {
        ...
        SDKHook(client, SDKHook_TraceAttack, OnTraceAttackHeadshot);
    }

    public void OnPluginEnd()
    {
        if (g_hTankDamageKVs != INVALID_HANDLE)
        {
            CloseHandle(g_hTankDamageKVs);
            g_hTankDamageKVs = INVALID_HANDLE;
        }

        if (g_hHeadshotKVs != INVALID_HANDLE)
        {
            CloseHandle(g_hHeadshotKVs);
            g_hHeadshotKVs = INVALID_HANDLE;
        }
    ...
    public Action Command_Weapon(int args)
    {
        if (args < 3)
        {
            PrintToServer("Usage: sm_weapon <weapon> <attr> <value>");
        ...
        {
            PrintToServer("Invalid weapon name: %s", sWeaponName);
            return Plugin_Handled;
        }

        int iAttr = GetWeaponAttributeIndex(sAttrName);
        if (iAttr == -1)
        {
            PrintToServer("Invalid attribute name: %s", sAttrName);
            return Plugin_Handled;
        }

        Format(sWeaponFull, sizeof(sWeaponFull), "weapon_%s", sWeaponName);

        if (iAttr < 3)
        {
            int iValue = StringToInt(sAttrValue);
            L4D2_SetIntWeaponAttribute(sWeaponFull, view_as<L4D2IntWeaponAttributes>(g_iWeaponAttributes[iAttr]), iValue);
            PrintToServer("%s for %s set to %d", g_sWeaponAttrNames[iAttr], sWeaponName, iValue);
        }
        else if (iAttr < MAX_ATTRS - 1)
        {
            float fValue = StringToFloat(sAttrValue);
            L4D2_SetFloatWeaponAttribute(sWeaponFull, view_as<L4D2FloatWeaponAttributes>(g_iWeaponAttributes[iAttr]), fValue);
            PrintToServer("%s for %s set to %.2f", g_sWeaponAttrNames[iAttr], sWeaponName, fValue);
        }
        else
        {
            float fValue = StringToFloat(sAttrValue);
            KvSetFloat(g_hTankDamageKVs, sWeaponFull, fValue);
            PrintToServer("%s for %s set to %.2f", g_sWeaponAttrNames[iAttr], sWeaponName, fValue);
        }
    ...
    */
        ...
        if (IsSupportedMelee(sWeaponName)) {
            ...
            if (iAttrIdx < INT_MELEE_MAX_ATTRS) {
                ...
                if (iAttrIdx < GAME_MELEE_MAX_ATTRS) {
                    ...
                    if (fValue <= 0.0) {
                        KvSetFloat(g_hTankDamageKVs, sWeaponName, 1.0); // valor por defecto = 1.0 (daño normal)
                        PrintToServer("Tank Damage Multiplier (tankdamagemult) reset for %s melee!", sWeaponName);
                    } else {
                        KvSetFloat(g_hTankDamageKVs, sWeaponName, fValue);
            ...
            else
            {
                ...
                else if (StrEqual(sAttrName, "headshotmult", false))
                {
                    if (fValue <= 0.0)
                    {
                        KvSetFloat(g_hHeadshotKVs, sWeaponName, 1.0);
                        PrintToServer("Headshot Multiplier (headshotmult) reset for %s weapon!", sWeaponName);
                    }
                    else
                    {
                        KvSetFloat(g_hHeadshotKVs, sWeaponName, fValue);
                ...
                else if (StrEqual(sAttrName, "reloaddurationmult", false))
                {
                    ...
                    if (fValue <= 0.0)
                    {
                        ...
                        {
                            PrintDebug(eLogError|eServerPrint, "Check weapon attribute '%s' value, cannot be set below zero or zero. Set the value: %f!", sAttrName, fValue);
                            return Plugin_Handled;
                        }
    ...
    Action Cmd_WeaponAttributes(int client, int args)
    {
        ...
        if (IsSupportedMelee(sWeaponName)) {
            ...
            float fBuff = KvGetFloat(g_hTankDamageKVs, sWeaponName, 1.0);
            ...
            fBuff = KvGetFloat(g_hHeadshotKVs, sWeaponName, 1.0);
            CReplyToCommand(client, "- {lightgreen}Headshot Multiplier{default}: {olive}%.2f", fBuff);
    ...
    public Action Command_WeaponAttributes(int client, int args)
    {
        if (args < 1)
        {
            ReplyToCommand(client, "Usage: sm_weapon_attributes <weapon>");
            return Plugin_Handled;
        }
        ...
        if (!L4D2_IsValidWeapon(sWeaponName))
        {
            ReplyToCommand(client, "Invalid weapon name: %s", sWeaponName);
            return Plugin_Handled;
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            int iValue = L4D2_GetIntWeaponAttribute(sWeaponFull, view_as<L4D2IntWeaponAttributes>(g_iWeaponAttributes[i]));
            ReplyToCommand(client, "%s: %d", g_sWeaponAttrNames[i], iValue);
        }

        for (int i = 3; i < MAX_ATTRS - 1; i++)
        {
            float fValue = L4D2_GetFloatWeaponAttribute(sWeaponFull, view_as<L4D2FloatWeaponAttributes>(g_iWeaponAttributes[i]));
            ReplyToCommand(client, "%s: %.2f", g_sWeaponAttrNames[i], fValue);
        }

        float fBuff = KvGetFloat(g_hTankDamageKVs, sWeaponFull, 0.0);
        if (fBuff > 0.0)
            ReplyToCommand(client, "%s: %.2f", g_sWeaponAttrNames[MAX_ATTRS - 1], fBuff);
    ...
    */
    ...
    {
        // Reinicia todos los multiplicadores vs Tank
        if (g_hTankDamageKVs != INVALID_HANDLE) {
            CloseHandle(g_hTankDamageKVs);
        }
        g_hTankDamageKVs = CreateKeyValues("DamageVsTank");
        PrintToServer("L4D2 Weapon Attributes: tank damage multipliers cleared.\n");
        ...
        g_hHeadshotKVs = CreateKeyValues("HeadshotMultiplier");
        PrintToServer("L4D2 Weapon Attributes: headshot damage multipliers cleared.\n");

        // Reinicia los multiplicadores de recarga
        ...
        if (iWeaponAttrCount || iMeleeAttrCount) {
            PrintToServer("The weapon attributes for all saved weapons have been reset successfully. Number of reset weapon attributes: %d!\n", iWeaponAttrCount + iMeleeAttrCount);
        }
   ...
   public Action OnTraceAttackHeadshot(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &ammotype, int hitbox, int hitgroup)
    ...
    public Action DamageBuffVsTank(int victim, int &attacker, int &inflictor, float &damage, int &damageType, int &weapon, float damageForce[3], float damagePosition[3])
    {
        ...
        // Validate if the victim is a Tank
        if (!IsTank(victim))
            return Plugin_Continue;

        // Get the attacker's active weapon
        char sWeapon[64];
        GetClientWeapon(attacker, sWeapon, sizeof(sWeapon));

        // Normalize the weapon name: remove "weapon_" prefix if present
        if (strncmp(sWeapon, "weapon_", 7) == 0)
        {
            strcopy(sWeapon, sizeof(sWeapon), sWeapon[7]);
        }

        // Get the tank damage multiplier from KeyValues (default 0.0)
        float fBuff = KvGetFloat(g_hTankDamageKVs, sWeapon, 0.0);

        // Apply the multiplier only if it's defined (non-zero)
        if (fBuff != 0.0)
        {
            damage *= fBuff;
            return Plugin_Changed; // Indicates the damage was modified
        }

        return Plugin_Continue; // No modification applied
    }
    ...
    bool IsTank(int client)
    {
        ...
        return (zombieClass == TANK_ZOMBIE_CLASS);
    }
