...
Handle g_hDragTimers[MAXPLAYERS + 1]; // Timer de daño por jugador
...
public Action Timer_ManualDragDamage(Handle timer)
{
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		int attacker = GetEntPropEnt(i, Prop_Send, "m_tongueOwner");
		if (attacker > 0 && IsClientInGame(attacker) && GetClientTeam(attacker) == TEAM_INFECTED)
		{
			if (IsSurvivorBeingDragged(i))
			{
				// Manually Replicate Versus Damage
				float dmg = tongue_drag_damage_amount.FloatValue;
				SDKHooks_TakeDamage(i, attacker, attacker, dmg);
			}
		}
	}
...
Action FirstDamage(Handle hTimer, any userid)
{
	...
	if (client > 0 && GetClientTeam(client) == TEAM_SURVIVOR && IsSurvivorBeingDragged(client))
	{
		...
		// Coop FIX for drag damage over time
		if (!IsVersusMode())
		{
			KillDragTimer(client);
			float fInterval = tongue_drag_damage_interval.FloatValue;

			g_hDragTimers[client] = CreateTimer(fInterval, Timer_ManualDragDamage, GetClientUserId(client), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
		}
...
void KillDragTimer(int client)
{
	if (g_hDragTimers[client] != null)
	{
		KillTimer(g_hDragTimers[client]);
		g_hDragTimers[client] = null;
	}
}

bool IsVersusMode()
{
    static Handle g_hVersus = INVALID_HANDLE;
    if (g_hVersus == INVALID_HANDLE)
        g_hVersus = FindConVar("mp_gamemode");

    char gamemode[32];
    GetConVarString(g_hVersus, gamemode, sizeof(gamemode));
