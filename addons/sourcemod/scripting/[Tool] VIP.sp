...
Action Command_VoteBlocker(int client, char [] command, int args)
{
	...
	{
		PrintToChat(client, "\x04[Vote Blocker] \x01This feature is \x05blocked\x01.");
		return Plugin_Handled;
	}
...
public Action OnClientSayCommand(int client, const char[] command, const char[] sArgs)
{
	...
	if (strcmp(command, "say_team", false) != 0)
	{
		...
		for (int i = 1; i <= MaxClients; i++) 
		{
			...
			else if(g_bText_Red[client]) SayText2(i,  FindRandomPlayersByTeam(3), "\x03[FF]^ \x01%N \x01:  %s", client, sArgs);
			else if(g_bText_White[client]) SayText2(i, FindRandomPlayersByTeam(3), "\x03[FF]^ \x06%N \x01:  %s", client, sArgs);
			else SayText2(i, FindRandomPlayersByTeam(3), "\x05[FF]^ \x04%N \x03:  %s", client, sArgs);
		}
	}
	else if (strcmp(command, "say_team", true) != -1)
	{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			...
			if(GetClientTeam(i) == GetClientTeam(client))
			{
				...
				else if(g_bText_White[client]) SayText2(i, client, "\x03%N \x01:  %s", sTeam, client, sArgs);
