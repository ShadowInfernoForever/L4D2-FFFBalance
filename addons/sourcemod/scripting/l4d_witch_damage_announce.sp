...
void WitchDeath_Event(Event hEvent, const char[] name, bool dontBroadcast)
{
    ...
    {
        CPrintToChatAll("{default}[{green}!{default}] El {red}Tank {default}({olive}%N{default}) mató a la {red}Witch", killer);
        return;
    }
...
void PrintWitchDamage(int witch, bool witchAlive = false)
{
    ...
    if (witchAlive)
        CPrintToChatAll("{default}[{green}!{default}] La {blue}Witch {default}tenia {olive}%d {default}de vida restante", witchMaxHealth - damageWitchTotal[witch]);
    else
        CPrintToChatAll("{default}[{green}!{default}] {blue}Daño {default}hecho a la {blue}Witch:");
    ...
    for (int k; k < survivorIndex; k++)
    {
        ...
        */
        ...
        for (int i = 1; i <= MaxClients; i++)
        {
            if (IsClientInGame(i))
            {
                CPrintToChat(i, "{blue}[{default}%d{blue} dmg] ({default}%i%%{blue}) {olive}%N", damage, percentDamage, client);
            }
        }
    }
