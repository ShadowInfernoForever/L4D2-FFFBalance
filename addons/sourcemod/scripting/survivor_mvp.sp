        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_CI_losers[i]) && !IsFakeClient(mvp_CI_losers[i])) 
            {
                if (bSolo)
                {
                    ...
                    {
                        Format(tmpBuffer, sizeof(tmpBuffer), "{blue}Rango {green}CI{default}: {olive}#%d {blue}({default}%d {green}comunes {blue}[{default}%.0f%%{blue}])", (i + 2), iGotCommon[mvp_CI_losers[i]], (float(iGotCommon[mvp_CI_losers[i]]) / float(iTotalCommon)) * 100);
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_FF_losers[i]) &&  !IsFakeClient(mvp_FF_losers[i])) 
            {
                if (bSolo)
                {
                    ...
                    {
                        Format(tmpBuffer, sizeof(tmpBuffer), "{blue}Rango {green}FF{default}: {olive}#%d {blue}({default}%d {green}fuego amigo {blue}[{default}%.0f%%{blue}])", (i + 2), iDidFF[mvp_FF_losers[i]], (float(iDidFF[mvp_FF_losers[i]]) / float(iTotalFF)) * 100);
...
void GetMVPString(char[] printBuffer, const int iSize)
{
    ...
    else
    {
        if (!(iBrevityFlags & BREV_SI))
        {
            ...
            {
                StrCat(printBuffer, iSize, "{blue}[{default}MVP{blue}] SI: {blue}({default}nadie{blue}){default}\n");
            }
        }
