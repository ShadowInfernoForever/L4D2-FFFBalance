...
{
	CreateConVar("l4d_ff_announce_version", "1.4", "FF announce Version", FCVAR_NOTIFY);
	FFenabled = CreateConVar("l4d_ff_announce_enable", "1", "Enable Announcing Friendly Fire", FCVAR_NOTIFY);
	AnnounceType = CreateConVar("l4d_ff_announce_type", "1", "Changes how ff announce displays FF damage (1:In chat; 2: In Hint Box; 3: In center text)", FCVAR_NOTIFY);
		...
		if (DamageCache[attackerc][i] != 0 && attackerc != i)
		{
			if (IsClientInGame(i) && IsClientConnected(i))
			{
				...
				switch(GetConVarInt(AnnounceType))
				{
					...
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
							CPrintToChat(attackerc, "{green} • {blue}Tu {default}Hiciste {blue}%d {olive}FF-DMG {default}A {blue}%s",DamageCache[attackerc][i],victim);
						if (IsClientInGame(i) && IsClientConnected(i) && !IsFakeClient(i))
							CPrintToChat(i, "{green} • {blue}%s {default}Hizo {blue}%d {olive}FF-DMG {default}Hacia {blue}Ti",attacker,DamageCache[attackerc][i]);
					}
					case 2:
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
							PrintHintText(attackerc, "[Sos un bobi] You did %d friendly fire damage to %s",DamageCache[attackerc][i],victim);
						if (IsClientInGame(i) && IsClientConnected(i) && !IsFakeClient(i))
							PrintHintText(i, "[Alto Bobi] %s did %d friendly fire damage to you",attacker,DamageCache[attackerc][i]);
					}
					case 3:
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
							PrintCenterText(attackerc, "[Sos un bobi] You did %d friendly fire damage to %s",DamageCache[attackerc][i],victim);
						if (IsClientInGame(i) && IsClientConnected(i) && !IsFakeClient(i))
							PrintCenterText(i, "[Alto Bobi] %s did %d friendly fire damage to you",attacker,DamageCache[attackerc][i]);
					}
				}
			}
