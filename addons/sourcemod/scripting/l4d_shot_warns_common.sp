...
#define CLASSNAME_PIPE_BOMB_PROJECTILE         "pipe_bomb_projectile"

#define CLASSNAME_PROP_MINIGUN                 "prop_minigun"
#define CLASSNAME_PROP_MINIGUN_L4D1            "prop_minigun_l4d1"
#define CLASSNAME_PROP_MOUNTED_MACHINE_GUN     "prop_mounted_machine_gun"
...
static ConVar g_hCvar_Vocalize;
...
// ====================================================================================================
static bool   ge_bIsPipeBombProjecitle[MAXENTITIES+1];
...
public void OnEntityCreated(int entity, const char[] classname)
{
    ...
    switch (classname[0])
    {
        ...
        case 'p':
        {
            if (StrEqual(classname, CLASSNAME_PIPE_BOMB_PROJECTILE))
...
public Action VocalizeCommand(int client, const char[] command, int argc)
{
    if (!g_bCvar_Vocalize)
        return Plugin_Continue;
...
public void ValidateChase(int client)
{
    ...
    else
    {
        switch (weaponid)
        {
            ...
            case L4D1_WEPID_MACHINE_GUN:
            {
                ...
                if (StrEqual(classname, CLASSNAME_PROP_MOUNTED_MACHINE_GUN))
                {
                    if (!g_bCvar_50Cal)
                        return false;
                }
                else if (StrEqual(classname, CLASSNAME_PROP_MINIGUN))
                {
                    if (!g_bCvar_Minigun)
                        return false;
                }
