...
{ 
	...
	g_hCvarAssaultReminderInterval 	= CreateConVar( "ai_assault_reminder_interval", "7", 	"Frequency(sec) at which the 'nb_assault' command is fired to make SI attack", CVAR_FLAGS, true, 0.0 );
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon) {
	...
	{ 
		...
		case (L4D2Infected_Smoker): {
				return Smoker_OnPlayerRunCmd( client, buttons);
			}
