...
void KillDominatorAndReportRemainingHealth(infected)
{
    ...
    CPrintToChatAll(
    "[{olive}capclear{default}] {red}%N{default} tenía {olive}%d{default} de vida restante.",
...
void ReportCleared()
{
    CPrintToChatAll("[{olive}capclear{default}] {red}¡Liberación completa!{default} Liberando capturadores para dar a los supervivientes más oportunidad de luchar.");

    if (iClearsLeftThisRound == 0) {
        PrintHintTextToAll("¡Supervivientes liberados!\n¡Cuidado! La próxima vez será el fin del juego.");
    } else if (iClearsLeftThisRound > 0) {
        PrintHintTextToAll("¡Supervivientes liberados!\nLiberaciones restantes: %d", iClearsLeftThisRound);
    } else {
        PrintHintTextToAll("¡Supervivientes liberados!");
    }
}
