...
#define SPAWN_ALERT			"ui/pickup_secret01.wav"
...
public OnClientPostAdminCheck(client)
{
	...
	if (GetConVarBool(g_hEnable))
	{
		...
		if (IsClientInGame(client) && !IsFakeClient(client))
		{
			...
			//CPrintToChat(client, "Welcome to Tank Rush 2! (v%s)", VERSION);

			// If hardcore mode is enabled, continue
			if (GetConVarBool(g_hHardcoreMode))
			{
				// Print message notifying them
				CPrintToChat(client, "%s HARDCORE mode is enabled!", PREFIX);
			}
			...
			if (StrEqual(g_sGameMode, "coop", false) && g_bCoopStart)
			{
				...
				CPrintToChat(client, "%s {default}Los tanks spawnean en %d segundos!", PREFIX, iSeconds);
			}
		}
		...
		if (IsClientInGame(client) && IsClientAuthor(client))
		{
			...
			CPrintToChatAll("%s Developer of Tank Rush 2 has joined!", PREFIX);
		}
	}
}
...
public OnMapStart()
{
	PrecacheSound(SPAWN_ALERT, true);
	...
	if (GetConVarBool(g_hEnable))
	{
		...
		if (g_bCoopStart)
		{
			...
			if (g_iCoopStartTick >= GetConVarInt(g_hCoopStart))
			{
				...
				g_bCoopStart = false;
				g_iCoopStartTick = 0;
				CPrintToChatAll("{red}[{default}!{red}] {default}Los {olive}Tanks {default}comienzan aparecer...");
				EmitSoundToAll(SPAWN_ALERT, SNDCHAN_WEAPON, SNDLEVEL_SCREAMING);
			}
		}
...
public Action:Timer_Advert(Handle:timer)
{
	...
	if (GetConVarBool(g_hEnable))
	{
		...
		//CPrintToChatAll("%s Join the official Tank Rush 2 steam group!", PREFIX);
		//CPrintToChatAll("%s steamcommunity.com/groups/tankrush2", PREFIX);
		return;
	}
}
...
{
    if (LeftStartArea() && GetConVarBool(g_hEnable) && !StrEqual(g_sGameMode, "coop", false) ) 
    { 
    	g_bSpawning = true;
		CPrintToChatAll("{red}[{default}!{red}]{default}Los {olive}Tanks {default}comienzan aparecer...");
		EmitSoundToAll(SPAWN_ALERT, SNDCHAN_WEAPON, SNDLEVEL_SCREAMING);
