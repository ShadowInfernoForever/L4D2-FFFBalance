...
public OnPluginStart()
{
	...
	CreateConVar("tr_version", VERSION, "Version of the installed plugin", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_DONTRECORD|FCVAR_NOTIFY);

	// Create the rest of the convars for the plugin
	g_hEnable							= CreateConVar("tr_enable",			"1",	"0 - Disable plugin, 1 - Enable plugin",																					FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hSpawnInterval					= CreateConVar("tr_spawninterval",	"6",	"Time in seconds between tank spawns",																						FCVAR_PLUGIN, true, 1.0, true, 180.0);
	g_hTankHealth						= CreateConVar("tr_tankhealth",		"4000",	"Amount of health tanks will spawn with",																					FCVAR_PLUGIN, true, 1.0, true, 25000.0);
	g_hTankSpeed						= CreateConVar("tr_tankspeed",		"1.08",	"Ratio for bot and player tank speed",																						FCVAR_PLUGIN, true, 0.0, true, 5.0);
	g_hTankLimit						= CreateConVar("tr_tanklimit",		"10",	"Maximum number of tanks allowed to spawn at a given time",																	FCVAR_PLUGIN, true, 1.0, true, 10.0);
	g_hCommonLimit						= CreateConVar("tr_commonlimit",	"0",	"Maximum number of common allowed to spawn at a given time",																FCVAR_PLUGIN, true, 0.0, true, 100.0);
	g_hGiveHealth						= CreateConVar("tr_givehealth",		"2",	"0 - Tank kills do not give health, 1 - Tank kills heal all players, 2 - Tank kills heal player with least health",			FCVAR_PLUGIN, true, 0.0, true, 2.0);
	g_hCountdown						= CreateConVar("tr_countdown",		"240",	"Time in seconds the tanks have to kill the survivors",																		FCVAR_PLUGIN, true, 60.0, true, 600.0);
	g_hCooldown							= CreateConVar("tr_cooldown",		"30",	"Time in seconds for resting periods on finales",																			FCVAR_PLUGIN, true, 1.0, true, 60.0);
	g_hHardcoreMode						= CreateConVar("tr_hardcoremode",	"0",	"0 - Disable hardcore mode, 1 - Enable hardcore mode",																		FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public OnClientPostAdminCheck(client)
{
	...
	if (GetConVarBool(g_hEnable))
	{
		...
		if (IsClientInGame(client) && !IsFakeClient(client))
		{
			...
			PrintToChat(client, "%s Bienvenido a Tank Rush Versus! (v%s)", PREFIX, VERSION);

			// If hardcore mode is enabled, continue
			if (GetConVarBool(g_hHardcoreMode))
			{
				// Print message notifying them
				PrintToChat(client, "%s modo HARDCORE Habilitado!", PREFIX);
			}
	...
	if (GetConVarBool(g_hEnable))
	{
		...
		for (new x = 1; x <= MaxClients; x++)
		{
			...
			if (IsClientInGame(x) && !IsFakeClient(x) && IsPlayerGhost(x) && GetClientTeam(x) == 3 && g_bWaiting[x])
			{
				...
				PrintCenterText(client, "EL SPAWN DE TANKS SERÁ EN %d Segundos!", iSeconds);
				...
				if (g_iWaitingTick[x] >= GetConVarInt(g_hSpawnInterval))
				{
					...
					// Notify the client
					PrintCenterText(client, "PUEDEN RESPAWNEAR!");
...
public Action:Event_PlayerSpawn(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	if (GetConVarBool(g_hEnable))
	{
		...
		if (!GetConVarBool(g_hCollision))
		{
			...
			new m_CollisionGroup = FindSendPropOffs("CBaseEntity", "m_CollisionGroup");
