...
1.1.3 (24-July-2021)
   - Fixed bar not showing up when the current HP is bigger than max HP.
...
1.1.0 (22-February-2021)
    ...
    - Added cvar to remove blur effect behind walls. (thanks "KadabraZz" for requesting)
...
ConVar g_hCvar_AliveShow;
ConVar g_hCvar_AliveModel;
ConVar g_hCvar_AliveAlpha;
ConVar g_hCvar_AliveScale;
ConVar g_hCvar_DeadShow;
ConVar g_hCvar_DeadModel;
ConVar g_hCvar_DeadAlpha;
ConVar g_hCvar_DeadScale;
ConVar g_hCvar_DeadColor;
ConVar g_hCvar_RemoveBlur;
ConVar g_hCvar_Pulse;
...
ConVar g_hCvar_CustomModel;
ConVar g_hCvar_CustomModelVMT;
ConVar g_hCvar_CustomModelVTF;
ConVar g_hCvar_SI;

//Custom
ConVar g_hCvar_IncapacitatedVMT;
ConVar g_hCvar_IncapacitatedVTF;

ConVar g_hCvar_LastStandVMT;
ConVar g_hCvar_LastStandVTF;

ConVar g_hCvar_LastStandColor;
...
bool g_bCvar_AliveShow;
bool g_bCvar_DeadShow;
bool g_bCvar_RemoveBlur;
bool g_bCvar_BlackAndWhite;
bool g_bCvar_ClientAlphaMax;
bool g_bCvar_CustomModel;
...
int g_iCvar_AliveAlpha;
int g_iCvar_DeadAlpha;
int g_iCvar_Pulse;
int g_iCvar_Team;
...
float g_fCvar_RenderInterval;
float g_fCvar_ZAxis;
float g_fCvar_AttackDelay;
float g_fCvar_AliveScale;
float g_fCvar_DeadScale;

// ====================================================================================================
// string - Plugin Variables
// ====================================================================================================
char g_sCvar_AliveModel[PLATFORM_MAX_PATH];
char g_sCvar_AliveAlpha[4];
char g_sCvar_AliveScale[5];
char g_sCvar_DeadModel[PLATFORM_MAX_PATH];
char g_sCvar_DeadAlpha[4];
char g_sCvar_DeadScale[5];
char g_sCvar_DeadColor[12];
char g_sCvar_FadeDistance[5];
char g_sCvar_CustomModelVMT[PLATFORM_MAX_PATH];
char g_sCvar_CustomModelVTF[PLATFORM_MAX_PATH];

//Custom

char g_sCvar_IncapacitatedVMT[PLATFORM_MAX_PATH];
char g_sCvar_IncapacitatedVTF[PLATFORM_MAX_PATH];

char g_sCvar_LastStandVMT[PLATFORM_MAX_PATH];
char g_sCvar_LastStandVTF[PLATFORM_MAX_PATH];

char g_sCvar_LastStandColor[12];
...
int gc_iSpriteInfoTargetEntRef[MAXPLAYERS+1] = { INVALID_ENT_REFERENCE, ... };
...
public void OnPluginStart()
{
    ...
    g_hCvar_AliveScale        = CreateConVar("l4d_hp_sprite_alive_scale", "0.30", "Scale of alive sprite (increases both height and width).\nNote: Some range values maintain the same size. (e.g. from 0.0 to 0.38 the size doesn't change).", CVAR_FLAGS, true, 0.0);
    g_hCvar_DeadShow          = CreateConVar("l4d_hp_sprite_dead_show", "1", "Show the dead sprite when a client dies.\n0 = OFF, 1 = ON.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_DeadModel         = CreateConVar("l4d_hp_sprite_dead_model", "sprites/death_icon.vmt", "Model of dead sprite.");
    ...
    g_hCvar_SurvivorTeam      = CreateConVar("l4d_hp_sprite_survivor_team", "1", "Which teams survivors can see the sprite.\n0 = NONE, 1 = SURVIVOR, 2 = INFECTED, 4 = SPECTATOR, 8 = HOLDOUT.\nAdd numbers greater than 0 for multiple options.\nExample: \"3\", enables for SURVIVOR and INFECTED.", CVAR_FLAGS, true, 0.0, true, 15.0);
    g_hCvar_InfectedTeam      = CreateConVar("l4d_hp_sprite_infected_team", "2", "Which teams infected can see the sprite.\n0 = NONE, 1 = SURVIVOR, 2 = INFECTED, 4 = SPECTATOR, 8 = HOLDOUT.\nAdd numbers greater than 0 for multiple options.\nExample: \"3\", enables for SURVIVOR and INFECTED.", CVAR_FLAGS, true, 0.0, true, 15.0);
    ...
    g_hCvar_CustomModel       = CreateConVar("l4d_hp_sprite_custom_model", "1", "Use a custom sprite for the alive model\nNote: This creates an extra entity for each sprite.\nNote: This requires the client downloading the custom model (.vmt and .vtf) to work.\nSearch for FastDL for more info.\n0 = OFF, 1 = ON.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_CustomModelVMT    = CreateConVar("l4d_hp_sprite_custom_model_vmt", "mart/mart_custombar2.vmt", "Custom sprite VMT path.");
    g_hCvar_CustomModelVTF    = CreateConVar("l4d_hp_sprite_custom_model_vtf", "mart/mart_custombar2.vtf", "Custom sprite VTF path.");

    //Custom
    g_hCvar_IncapacitatedVMT  = CreateConVar("l4d_hp_sprite_incapacitated_vmt", "mart/knockdown.vmt", "Custom sprite VMT path.");
    g_hCvar_IncapacitatedVTF  = CreateConVar("l4d_hp_sprite_incapacitated_vtf", "mart/knockdown.vtf", "Custom sprite VTF path.");

    g_hCvar_LastStandVMT      = CreateConVar("l4d_hp_sprite_laststand_vmt", "mart/last_stand.vmt", "Custom sprite VMT path.");
    g_hCvar_LastStandVTF      = CreateConVar("l4d_hp_sprite_laststand_vtf", "mart/last_stand.vtf", "Custom sprite VTF path.");

    g_hCvar_LastStandColor    = CreateConVar("l4d_hp_sprite_laststand_color", "225 225 255", "Color of last stand sprite.\nUse three values between 0-255 separated by spaces (\"<0-255> <0-255> <0-255>\").", CVAR_FLAGS);

    if (g_bL4D2)
        g_hCvar_SI            = CreateConVar("l4d_hp_sprite_si", "0", "Which special infected should have a sprite.\n1 = SMOKER, 2 = BOOMER, 4 = HUNTER, 8 = SPITTER, 16 = JOCKEY, 32 = CHARGER, 64 = TANK.\nAdd numbers greater than 0 for multiple options.\nExample: \"127\", enables sprite for all SI.", CVAR_FLAGS, true, 0.0, true, 127.0);
    else
    ...
    // Custom
    g_hCvar_IncapacitatedVMT.AddChangeHook(Event_ConVarChanged);
    g_hCvar_IncapacitatedVTF.AddChangeHook(Event_ConVarChanged);

    g_hCvar_LastStandVMT.AddChangeHook(Event_ConVarChanged);
    g_hCvar_LastStandVTF.AddChangeHook(Event_ConVarChanged);

    g_hCvar_LastStandColor.AddChangeHook(Event_ConVarChanged);
...
void GetCvars()
{
    ...
    IntToString(g_iCvar_FadeDistance, g_sCvar_FadeDistance, sizeof(g_sCvar_FadeDistance));
    ...
    IntToString(g_iCvar_AliveAlpha, g_sCvar_AliveAlpha, sizeof(g_sCvar_AliveAlpha));
    ...
    IntToString(g_iCvar_DeadAlpha, g_sCvar_DeadAlpha, sizeof(g_sCvar_DeadAlpha));
    g_fCvar_DeadScale = g_hCvar_DeadScale.FloatValue;
    FloatToString(g_fCvar_DeadScale, g_sCvar_DeadScale, sizeof(g_sCvar_DeadScale));
    ...
    // Custom
    g_hCvar_IncapacitatedVMT.GetString(g_sCvar_IncapacitatedVMT, sizeof(g_sCvar_IncapacitatedVMT));
    TrimString(g_sCvar_IncapacitatedVMT);

    g_hCvar_IncapacitatedVTF.GetString(g_sCvar_IncapacitatedVTF, sizeof(g_sCvar_IncapacitatedVTF));
    TrimString(g_sCvar_IncapacitatedVTF);

    g_hCvar_LastStandVMT.GetString(g_sCvar_LastStandVMT, sizeof(g_sCvar_LastStandVMT));
    TrimString(g_sCvar_LastStandVMT);

    g_hCvar_LastStandVTF.GetString(g_sCvar_LastStandVTF, sizeof(g_sCvar_LastStandVTF));
    TrimString(g_sCvar_LastStandVTF);

    g_hCvar_LastStandColor.GetString(g_sCvar_LastStandColor, sizeof(g_sCvar_LastStandColor));
    TrimString(g_sCvar_LastStandColor);
    ...
    if (g_bCvar_AliveShow)
    {
        if (g_bCvar_CustomModel)
        {
            ...
            PrecacheModel(g_sCvar_IncapacitatedVMT, true);
            PrecacheModel(g_sCvar_LastStandVMT, true);
        }
        else
        {
...
void RenderHealthBar()
{
    for (int target = 1; target <= MaxClients; target++)
    {
        ...
        if (g_bCvar_RemoveBlur)
        {
            ...
            {
                infoTarget = CreateEntityByName("info_target");
        ...
        if (isIncapacitated && targetTeam == TEAM_SURVIVOR)
        {
            DispatchKeyValue(entity, "model", g_sCvar_IncapacitatedVMT);
            DispatchKeyValue(entity, "rendercolor", g_sCvar_DeadColor);
            DispatchKeyValue(entity, "renderamt", g_sCvar_DeadAlpha); // If renderamt goes before rendercolor, it doesn't render
            DispatchKeyValue(entity, "scale", g_sCvar_DeadScale);
            DispatchKeyValue(entity, "fademindist", g_sCvar_FadeDistance);
            DispatchKeyValue(entity, "renderfx", g_hCvar_Pulse ? "4" : "0");
            DispatchSpawn(entity);
        ...
        if (IsPlayerBlackAndWhite(target) && targetTeam == TEAM_SURVIVOR)
        {
            DispatchKeyValue(entity, "model", g_sCvar_LastStandVMT);
            DispatchKeyValue(entity, "rendercolor", g_sCvar_LastStandColor);
            DispatchKeyValue(entity, "renderamt", g_sCvar_DeadAlpha); // If renderamt goes before rendercolor, it doesn't render
            DispatchKeyValue(entity, "scale", g_sCvar_DeadScale);
            DispatchKeyValue(entity, "fademindist", g_sCvar_FadeDistance);
            DispatchKeyValue(entity, "renderfx", g_hCvar_Pulse ? "4" : "0");
        ...
        if (g_bCvar_ClientAlphaMax && colorAlpha[3] <= g_iCvar_ClientAlphaMax)
            ...
            IntToString(RoundFloat(g_iCvar_AliveAlpha * colorAlpha[3] / 255.0), sAlpha, sizeof(sAlpha));
        else
...
bool ShouldRenderHP(int target)
{
    ...
    if (!(GetTeamFlag(targetTeam) & g_iCvar_Team))
        return false;
...
Action CmdPrintCvars(int client, int args)
{
    ...
    PrintToConsole(client, "l4d_hp_sprite_render_interval : %.1f", g_fCvar_RenderInterval);
    PrintToConsole(client, "l4d_hp_sprite_z_axis : %.1f", g_fCvar_ZAxis);
    ...
    PrintToConsole(client, "l4d_hp_sprite_attack_delay : %.1f (%s)", g_fCvar_AttackDelay, g_bCvar_AttackDelay ? "true" : "false");
    ...
    PrintToConsole(client, "l4d_hp_sprite_alive_scale : %.1f", g_fCvar_AliveScale);
    ...
    PrintToConsole(client, "l4d_hp_sprite_dead_scale : %.1f", g_fCvar_DeadScale);
    ...
    PrintToConsole(client, "l4d_hp_sprite_team : %i (SPECTATOR = %s | SURVIVOR = %s | INFECTED = %s | HOLDOUT = %s)", g_iCvar_Team,
    ...
    PrintToConsole(client, "l4d_hp_sprite_survivor_team : %i (SPECTATOR = %s | SURVIVOR = %s | INFECTED = %s | HOLDOUT = %s)", g_iCvar_SurvivorTeam,
    ...
    PrintToConsole(client, "l4d_hp_sprite_infected_team : %i (SPECTATOR = %s | SURVIVOR = %s | INFECTED = %s | HOLDOUT = %s)", g_iCvar_InfectedTeam,
    ...
    PrintToConsole(client, "l4d_hp_sprite_spectator_team : %i (SPECTATOR = %s | SURVIVOR = %s | INFECTED = %s | HOLDOUT = %s)", g_iCvar_SpectatorTeam,
    ...
    if (g_bL4D2)
    {
        PrintToConsole(client, "l4d_hp_sprite_si : %i (SMOKER = %s | BOOMER = %s | HUNTER = %s | SPITTER = %s | JOCKEY = %s | CHARGER = %s | TANK = %s)", g_iCvar_SI,
    ...
    {
        PrintToConsole(client, "l4d_hp_sprite_si : %i (SMOKER = %s | BOOMER = %s | HUNTER = %s | TANK = %s)", g_iCvar_SI,
