...
void CreateCorrection(int common, int index, int correction)
{
	...
	else
	{
		...
		DispatchKeyValue(entity, "fadeInDuration", "1");
		DispatchKeyValue(entity, "fadeOutDuration", "5");
		...
		switch( correction )
		{
			...
			case 5:		DispatchKeyValue(entity, "filename", "materials/correction/thirdstrike.raw");             // Thirdstrike
...
void MutantTeslaSetup(int common)
{
	...
	if( g_bLeft4Dead2 && g_iConfTeslaGlow )
	{
		...
		SetEntPropFloat(common, Prop_Send, "m_flModelScale", 1.5);
		SetEntProp(common, Prop_Send, "m_iGlowType", 3);
		SetEntProp(common, Prop_Send, "m_glowColorOverride", g_iConfTeslaGlowCol);
		SetEntPropFloat(common, Prop_Data, "m_flSpeed", 0.7);
		AcceptEntityInput(common, "StartGlowing");
	}
...
void TeslaShock(int common, int client)
{
	...
	if( g_bLeft4Dead2 )
	{
		...
		if( GetRandomInt(0, 1) )
			DispatchKeyValue(entity, "effect_name", PARTICLE_TESLA3);
		else
...
int CreateEnvSprite(int client, const char[] sColor)
{
	...
	DispatchKeyValue(entity, "scale", "4");
	DispatchSpawn(entity);
...
int CreateParticle(int client, int type)
{
	...
	switch( type )
	{
		...
		{
			if( g_bLeft4Dead2 )
				DispatchKeyValue(entity, "effect_name", PARTICLE_ELMOS);// Tesla
			else
				DispatchKeyValue(entity, "effect_name", PARTICLE_TESLA3);
		}
	}
	...
	switch( type )
	{
		...
		{
			// Refire
			SetVariantString("OnUser1 !self:Stop::0.5:-1");
			AcceptEntityInput(entity, "AddOutput");
