...
PrintRemainingHealth()
{
	...
	CPrintToChatAll("{default}[{green}!{default}] {default}El {blue}Tank {default}({olive}%s{default}) Tenia {green}%d {default}de {green}HP {default}restante", name, g_iLastTankHealth);
}
	...
	if (!bPrintedHealth)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if(g_iWasTank[i] > 0)
			{
				...
				CPrintToChatAll("{default}[{green}!{default}] {blue}Daño {default}Hecho al {blue}Tank {default}({olive}%s{default})", name);
				g_iWasTank[i] = 0;
			}
			else if(g_iWasTankAI > 0) 
				CPrintToChatAll("{default}[{green}!{default}] {blue}Daño {default}Hecho al {blue}Tank {default}({default}BOT({olive}AI{default}){default})");
			g_iWasTankAI = 0;
		}
	}
