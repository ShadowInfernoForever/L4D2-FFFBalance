...
#define PLUGIN_DESCRIPTION	"Replace HeartBeat to Drums Music, dynamic set blackWhite screen"
...
public void OnPluginStart() {
	...
	cDetect =		CreateConVar(PLUGIN_NAME ... "_detect", "30",		"interval of frame(s) to detect back and white screen.", FCVAR_NOTIFY);
	cReviveMax =	FindConVar	("survivor_max_incapacitated_count");
	cStriked =		CreateConVar(PLUGIN_NAME ... "_striked", "40",		"only set black and white screen when health less than this value", FCVAR_NOTIFY);
	cGlow =			CreateConVar(PLUGIN_NAME ... "_glow", "1",			"does make glow when gets black and white screen.", FCVAR_NOTIFY);
...
void SetSurvivorCorrectBlackNWhite(int client) {
	...
	if ( IsGoingToDie(client) ) {
		...
		if (bMusic)
		if ( health <= 45 ) {
			ChangeDrumMuisc(client, RoundToCeil( (health * 2)  / (sizeof(MUSIC_DRUMS) * 2) )  );
...
int GetReviveCount(int client) {
	return isHeartbeatExists ? L4D_GetPlayerReviveCount(client) : L4D_GetPlayerReviveCount(client);
}
