...
public void OnPluginStart()
{
	...
	cvarTextDead             = CreateConVar("l4d2_infected_hudhpbar_textdead", "Critical Hit!", "Set text for dead hint (\"\" = no dead hint)");
    ...
    g_hCvar_pain_pills_decay_rate.AddChangeHook(CvarChanged_Activated);
...
void SettingHudHPBar(int client){
	...
	hp += GetClientTempHealth(client);
	float hpp = (float(hp) / float(hpmax)) * 100.0;
	int color[3];
	color=GetColorHP(hpp, RGB_YELLOW, RGB_GREEN, RGB_WHITE);
...
void SettingDeadHint(int client){
	...
	if(IsValidEnt(hint_target)){
		...
		if(IsValidEnt(hint)){
			...
			AcceptEntityInput(hint, "hint_shakeoption", 1);
			DispatchKeyValueFormat(hint, "hint_color", "%i %i %i", 0, 255, 0);
			AcceptEntityInput(hint, "ShowHint");
		}
	}else{
		hint_target = CreateHPBarHint(client, g_fShowTime, g_fRange, color, g_sTextDead);
		g_iHudTarget[client] = EntIndexToEntRef(hint_target);
	}
}
