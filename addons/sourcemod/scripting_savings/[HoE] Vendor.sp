...
#define SOUND_NEGATE		"buttons/button11.wav"
...
void CreateVendor(const float vOrigin[3], const float vAngles[3], int iType, int index = 0)
{
	...
	if( button != -1 )
	{
		...
		DispatchKeyValue(button, "use_string", "Consumiendo!");
		DispatchKeyValue(button, "use_sub_string", "Porfavor, recuerda dejar para tus compañeros");
...
public void OnTimeUp(const char[] output, int caller, int client, float delay)
{
	...
	float gHealth = GetEntPropFloat(client, Prop_Send, "m_healthBuffer");

        if( iHealth >= 50 ) {
    	CPrintToChat(client, "{red}• {default}Decides dejar algo para tu equipo.");
        EmitSoundToClient(client, SOUND_NEGATE );
        return;
        }
