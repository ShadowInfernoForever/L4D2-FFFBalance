...
#define pickup_1            "ui/gift_pickup.wav"
#define sColor              "200,200,200"
...
public void Event_ItemPickup(Event event, const char[] name, bool dontBroadcast)
{
	...
	for( int i = 0; i < MAX_FOOTLOCKERS; i++ )
	{
		...
		if( entity && (entity = EntRefToEntIndex(entity)) != INVALID_ENT_REFERENCE )
		{
			...
			{
				DupeItem(entity, i);
				EmitSoundToClient(client, pickup_1);
				return;
			}
...
void DupeItem(int entity, int index)
{
    ...
    //test
	//RemoveLocker(index);
    //RemoveEntity(g_iFootlockers[index][6]);
    int i, entity2;
    for( i = 0; i < 10; i++ )
	{
		entity2 = g_iFootlockers[index][6+i];
		//g_iFootlockers[index][i] = 0;

		if( IsValidEntRef(entity2) )
