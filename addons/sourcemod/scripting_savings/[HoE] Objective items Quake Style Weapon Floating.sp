...
{
	name = "Objective Items Quake Style Weapon Floating",
	author = "ZyLx",
...
public Phys_OnObjectSleep( entity )
{
	...
	if ( GetEntityClassname( entity, classname, sizeof( classname ) ) && strncmp( classname, "weapon_gascan", 7 ) == 0 )
	{
		Phys_EnableMotion( entity, g_bMotion );
		RequestFrame( SetEntPos, EntIndexToEntRef( entity ) );
	}

	if ( GetEntityClassname( entity, classname, sizeof( classname ) ) && strncmp( classname, "weapon_cola", 7 ) == 0 )
	{
		Phys_EnableMotion( entity, g_bMotion );
		RequestFrame( SetEntPos, EntIndexToEntRef( entity ) );
	}

	if ( GetEntityClassname( entity, classname, sizeof( classname ) ) && strncmp( classname, "weapon_gnome", 7 ) == 0 )
	{
		Phys_EnableMotion( entity, g_bMotion );
		RequestFrame( SetEntPos, EntIndexToEntRef( entity ) );
	}
...
public OnGameFrame()
{
	...
	for ( new entity = MaxClients + 1; entity < maxEnts; entity++ )
	{
		if ( IsValidEntity( entity ) && 
			 ...
			 strncmp( classname, "weapon_gascan", 7 ) == 0 )
		{
			GetEntPropVector( entity, Prop_Send, "m_vecOrigin", origin );
