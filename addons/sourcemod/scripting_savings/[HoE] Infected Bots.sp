...
bool g_bCvarAllow, g_bMapStarted, g_bSafeSpawn, g_bTankHealthAdjust, g_bVersusCoop, g_bScavengeCoop, g_bJoinableTeams, g_bCoopPlayableTank , g_bJoinableTeamsAnnounce,
...
public void OnPluginStart()
{
	...
	h_ScavengeCoop = 					CreateConVar("l4d_infectedbots_scavenge_coop", 							"0", 		"If 1, The plugin will force all players to the Survivor side against the Infected AI for every round and map in versus/scavenge", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_bScavengeCoop = h_ScavengeCoop.BoolValue;
	...
	h_ScavengeCoop.AddChangeHook(ConVarScavengeCoop);
...
public void ConVarScavengeCoop(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_bScavengeCoop = h_ScavengeCoop.BoolValue;
	if(L4D_HasPlayerControlledZombies() == true)
	{
...
Action Timer_PluginStart(Handle timer)
{
	...
	if (L4D_HasPlayerControlledZombies() && g_bScavengeCoop)
	{
		for (int i=1; i<=MaxClients; i++)
		{
			// We check if player is in game
	...
	if (g_bJoinableTeams && L4D_HasPlayerControlledZombies() == false || g_bScavengeCoop && L4D_HasPlayerControlledZombies())
	{
		if (g_bL4D2Version)
		{
			SetConVarInt(sb_all_bot_game, 1);
