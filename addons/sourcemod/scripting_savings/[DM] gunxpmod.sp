...
#define MAX_SECONDARY_LEVELS 26
...
public Action:Say_Command(client, args)
{
	...
	if( StrEqual(szArg1, "!nivel") || StrEqual(szArg1, "nivel") )
	{
		CPrintToChat(client, "{blue}NIVEL {default}[{green}%d{default}]", PlayerLevel[client] ) 
	...
	else if ( StrEqual(szArg1, "!armas") || StrEqual(szArg1, "armas") && GetConVarInt(CvarMenuReOpen) )
	{
		if ( !p_WeaponChoosen[client] )
		{
			CPrintToChat(client, "● {green}Abriendo menú, espera!" ) 
...
public MainWeaponMenu(client)
{
	...
	SetMenuTitle(menu, "● Elige tu principal");

	AddMenuItem(menu, "class_id", "➤ Elegir Arma Inicial")
	AddMenuItem(menu, "class_id", "➤ Última Arma Inicial Seleccionada")
...
public Action:Set_Player_Xp(client, value)
{
	...
	if( PlayerLevel[client] < MAX_LEVELS - 1 ) 
	{
		...
		if( PlayerXp[client] >= LEVELS[PlayerLevel[client]])
		{
			...
			CPrintToChat(client, "● Leveleaste a nivel {green}%d{default}!", PlayerLevel[client] ) 
		}
	}
