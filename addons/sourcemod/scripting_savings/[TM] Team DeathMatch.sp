...
new Handle:maxincapped;
new Handle:timegranade;
...
{
	name = "[L4D] Counter-Strike",
	author = "Ash The 9th Survivor / Pan Xiaohai",
	description = "Counter-Strike Mode to lado Oscuro Server",
...
public OnPluginStart()
{
	...
	l4d_cs_enable = CreateConVar("l4d_cs_enable", "1", "cs mode 0:disable, 1:enable ", FCVAR_NOTIFY);
	l4d_cs_respawntime = CreateConVar("l4d_cs_respawntime", "10", "", FCVAR_NOTIFY);
	l4d_cs_teambalance = CreateConVar("l4d_cs_teambalance", "1", "auto team balance  0:disable, 1:enable ", FCVAR_NOTIFY);
	l4d_cs_survivorglow = CreateConVar("l4d_cs_survivorglow", "0", "surivor glow  0:disable, 1:enable ", FCVAR_NOTIFY);
	l4d_cs_noinfected = CreateConVar("l4d_cs_noinfected", "0", "no infected 0:disable, 1:enable ", FCVAR_NOTIFY);	
	l4d_cs_alltalk = CreateConVar("l4d_cs_alltalk", "0", "voice communication between two team 0:disable, 1:enable ", FCVAR_NOTIFY);
	l4d_cs_deadalltalk = CreateConVar("l4d_cs_deadalltalk", "1", "dead all talk 0:disable, 1:enable ", FCVAR_NOTIFY);
	...
	/* Translations by Ash The 9th Survivor */
	LoadTranslations("counter_strike_mode.phrases");
	...
	maxincapped = FindConVar("survivor_max_incapacitated_count");
	timegranade = FindConVar("pipe_bomb_timer_duration");
	...
	AutoExecConfig(true, "[L4D] Counter-Strike");
...
public Action:TimerUpdate(Handle:timer)
{
	...
	if (GetConVarBool(l4d_cs_enable))
	{
		wason = 1;
		...
		SetConVarInt(maxincapped, 0);
		SetConVarInt(timegranade, 2);
	}
	else
	{
		if(wason)
		{
			...
			if ((client) && (startspawn == 1))
			{
				...
				SetConVarInt(maxincapped, 2);
				SetConVarInt(timegranade, 6);
				wason = 0;
			}
		}
	}
...
SwitchTeam(client)
{
	...
	if(full)
	{
		PrintCenterText(client, "%t", "team_full");
	}
	else
	{
...
public Action:sm_restart(client, args)
{
	...
	if(LeftSafeRoom)
	{
		...
		PrintHintTextToAll("%t", "game_restart");
	}
	else
	{
		PrintHintText(client, "%t", "left_area");
	}
	if(showhud_timer == INVALID_HANDLE )
	{
 		showhud_timer=CreateTimer(1.0, ShowHud, 0, TIMER_REPEAT);
		...
		for (new i = 1; i <= MaxClients; i++)
		{
			if(Teams[client]==Teams[i])
			{
				if (IsClientInGame(i) && GetClientTeam(i) ==2 )
				{
					if(IsPlayerAlive(i))
					{
						...
						SetListenOverride(i, client,alltalk);
					}
					else
					{
						SetListenOverride(client, i, LISTEN_YES);
			...
			else
			{
				if (IsClientInGame(i) && GetClientTeam(i) == 2 )
				{
					if(IsPlayerAlive(i))
					{
						SetListenOverride(client, i, alltalk);
						SetListenOverride(i, client, alltalk);
					}
					else
					{
						SetListenOverride(client, i, alltalk || deadalltalk );
						SetListenOverride(i, client, alltalk || deadalltalk );						
					}
				}
			}
...
public Action:ShowHud(Handle:timer, any:data)
{
	...
	if(!LeftSafeRoom)
	{
		...
		if(LeftSafeRoom)
		{
			PrintHintTextToAll("%t", "game_begin");
			RoundRestart();
		}
		return Plugin_Continue;
	}
	...
	SetPanelTitle(pInfHUD, "Puntuaciones:");
	...
	new goodteam2=0;
	new badteam2=0;
	Format(iStatus, sizeof(iStatus), "Anti-Terroristas (%d)", TeamWin[0]);
					...
					if(FlagTeam[j]!=Teams[i])
					{
						...
						if(dis<60.0)
						{
							...
							if(FlagValue[j]>=5 )
							{
								...
								if(FlagTeam[j]!=Teams[i])
								{
									PrintToChatAll("%t", "good_win_flag", 'A'+j);
							...
							else 
							{
								if(FlagValue[j]==-4 )
								{
									PrintToChatAll("%t", "bad_lost_flag", 'A'+j);
								}
								FlagTeam[j]=-1;
								EmitSoundToAll(SOUND_BLIP, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, FlagPoints[MapIndex][j], NULL_VECTOR, false, 0.0);
							}
							PrintHintText(i, "%t", "flag", 'A'+j);
						}
					}
				}
	...
	Format(iStatus, sizeof(iStatus), "Terroristas  (%d)", TeamWin[1]);
	DrawPanelText(pInfHUD, iStatus);
	for (i = 1; i <= MaxClients; i++)
	{
		if (!IsClientInGame(i)) continue;
					...
					if(FlagTeam[j]!=Teams[i])
					{
						...
						if(dis<60.0)
						{
							...
							if(FlagValue[j]<=-5)
							{
								...
								if(FlagTeam[j]!=Teams[i])
								{
									PrintToChatAll("%t", "bad_win_flag", 'A'+j);
									EmitSoundToAll(SOUND_BLIP, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, FlagPoints[MapIndex][j], NULL_VECTOR, false, 0.0);
							...
							else 
							{
								if(FlagValue[j]==4)
								{
									PrintToChatAll("%t", "good_lost_flag", 'A'+j);
								}
								FlagTeam[j]=-1;
								EmitSoundToAll(SOUND_BLIP, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, FlagPoints[MapIndex][j], NULL_VECTOR, false, 0.0);
							}
	...
	DrawPanelText(pInfHUD, " ");
	DrawPanelText(pInfHUD, "Puntos De Control:");
	Format(iStatus, sizeof(iStatus), "CT : ");
	...
	DrawPanelText(pInfHUD, iStatus);
	Format(iStatus, sizeof(iStatus), "TT  : ");
	for(i=0; i<FlagCount[MapIndex]; i++)
	{
		if(FlagTeam[i]==1)
		{
	...
	DrawPanelText(pInfHUD, iStatus);
	Format(iStatus, sizeof(iStatus), "Neutralidad : ");
	for(i=0; i<FlagCount[MapIndex]; i++)
	{
		if(FlagTeam[i]==-1)
		{
...
public Action:Win(Handle:Timer, any:team)
{
	...
	{
		wintick--;
		if(team==0)	PrintHintTextToAll("%t", "good_win", wintick);
		if(team==1)	PrintHintTextToAll("%t", "bad_win", wintick);
		return Plugin_Continue;
	}
	else
	{
