...
stock void SetNewName(int client, char[] name)
{
	...
	static char code3[4], code2[3];
	if(!(bLongTag ? GeoipCode3(ip, code3) : GeoipCode2(ip, code2)))
	{
		Format(name, MAX_NAME_LENGTH, "%s", name[GetPos(name)]);
        SetClientInfo(client, "name", name);
        return;
	}

	Format(name, MAX_NAME_LENGTH, "%c%s%c%s", HOOKS[0], bLongTag ? code3 : code2, HOOKS[1], name[GetPos(name)]);
	SetClientInfo(client, "name", name);
}
