    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_SKEET )
    {
        if ( attacker == -2 )
        {
            ...
            if ( IS_VALID_INGAME(victim) && !IsFakeClient(victim) ) {
            	CPrintToChatAll( "{green}★ {olive}%N {default}was team-skeeted.", victim );
            } else {
                PrintToChatAll( "\x04★ \x01A hunter was team-skeeted." );
            }
        }
...
HandleTongueCut( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_TONGUECUT )
    {
        ...
        {
            PrintToChatAll( "\x04★ \x05%N\x01 cut smoker tongue.", attacker );
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (    GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_SELFCLEAR &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04★ \x05%N\x01 self-cleared from \x05%N\x01's tongue%s.", attacker, victim, (withShove) ? " by shoving" : "" );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04★ \x05%N\x01 self-cleared from a smoker tongue%s.", attacker, (withShove) ? " by shoving" : "" );
        }
    }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_ROCKSKEET )
    {
        ...
        */
        PrintToChatAll( "\x04★ \x05%N\x01 skeeted a tank rock.", attacker );
    }
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_INSTACLEAR )
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                	CPrintToChatAll( "{green}★ {olive}%N{default} insta-cleared {olive}%N{default} from {green}%N{default} (%s) ({blue}%.2f {default}seconds).",
                            attacker, pinVictim, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                	CPrintToChatAll( "{green}★ {olive}%N{default} insta-cleared a teammate from {green}%N{default} (%s) ({blue}%.2f {default}seconds).",
                            attacker, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                	CPrintToChatAll( "{green}★ {olive}%N{default} insta-cleared {olive}%N{default} from a %s ({blue}%.2f {default}seconds).",
                            attacker, pinVictim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                	CPrintToChatAll( "{green}★ {olive}%N{default} insta-cleared a teammate from a %s ({blue}%.2f {default}seconds).",
                            attacker,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
...
stock HandleCarAlarmTriggered( survivor, infected, reason )
{
    if (    GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_CARALARM &&
        ...
        if ( reason == CALARM_HIT ) {
            PrintToChatAll( "\x04☠ \x05%N\x01 triggered an alarm with a hit.", survivor );
        }
        else if ( reason == CALARM_TOUCHED )
        {
            ...
            if ( IS_VALID_INFECTED(infected) )
            {
                ...
                {
                    PrintToChatAll( "\x04☠ \x05%N\x01 made \x05%N\x01 trigger an alarm.", infected, survivor );
                }
                else {
                    switch ( GetEntProp(infected, Prop_Send, "m_zombieClass") )
                    {
                        case ZC_SMOKER: { PrintToChatAll( "\x04☠ \x01A hunter made \x05%N\x01 trigger an alarm.", survivor ); }
                        case ZC_JOCKEY: { PrintToChatAll( "\x04☠ \x01A jockey made \x05%N\x01 trigger an alarm.", survivor ); }
                        case ZC_CHARGER: { PrintToChatAll( "\x04☠ \x01A charger made \x05%N\x01 trigger an alarm.", survivor ); }
                        default: { PrintToChatAll( "\x04☠ \x01A bot infected made \x05%N\x01 trigger an alarm.", survivor ); }
                    }
                }
            }
            else
            {
                PrintToChatAll( "\x04☠ \x05%N\x01 touched an alarmed car.", survivor );
            }
        }
        else if ( reason == CALARM_EXPLOSION ) {
            PrintToChatAll( "\x04☠ \x05%N\x01 triggered an alarm with an explosion.", survivor );
        }
        else if ( reason == CALARM_BOOMER )
        {
            if ( IS_VALID_INFECTED(infected) && !IsFakeClient(infected) )
            {
                PrintToChatAll( "\x04☠ \x05%N\x01 triggered an alarm by killing a boomer \x05%N\x01.", survivor, infected );
            }
            else
            {
                PrintToChatAll( "\x04☠ \x05%N\x01 triggered an alarm by shooting a boomer.", survivor );
            }
        }
        else {
            PrintToChatAll( "\x04☠ \x05%N\x01 triggered an alarm.", survivor );
        }
    }
